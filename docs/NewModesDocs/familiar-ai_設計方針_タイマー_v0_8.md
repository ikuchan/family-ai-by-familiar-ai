# familiar-ai 設計方針：タイマー（タイマー・ストップウォッチ）（v0.8）
...
## 10. 掛ける前に確かめる（`TIMER_CONFIRM`・v0.4・段 2）

既定 true。`set_timer` は `confirmed` 無しの呼び出しに「まだ掛けていない。本人に一度聞く：「3 分のタイマーね。その間は
黙って聞かないよ、いい？」」を返し、掛けない。確認の文は設定に合わせて変わる（`timer_rules.confirm_text`：黙る／聞かない
の有無）。「いい」なら `confirmed=true` で呼び直す（静穏時間・沈黙中の確認と同じ型。そちらは設定に関わらず常に）。
**調停でも主LLM でも同じ**——道具の返りをそのまま相手に言えばよい。
