# familiar-ai 設計方針：ループの語を束ねる（環-g・v0.7・全段 実装済み）
...
| **g-に** | **いま生きている記録を1本に**（②） | **変わる**・**完了**（2026-09-08） |
| **g-ほ** | ~~(id, 内容) の組（④）~~ | **取り下げ**（2026-09-08）。段に で片方が消え、揃える相手が無くなった |
| **g-へ** | **`_show_seeded` を消す**（⑤の付随） | **変わる**・**完了**（2026-09-08） |
...
### g-に の実装（2026-09-08・完了）

- `_chain_head_id` を撤去した。**読み手は `_advance_chain` の `if` だけ**だったので、
  `_advance_chain` ごと消えて誰も読まなくなった
- `_advance_chain` を撤去した。**`KIND_ADVANCE` を書く箇所が0件**になった（定数は残す）
- `_chain_head_content` は `_cue` へ改めた。実体は**この反復の手がかり＝いま生きている
  記録の内容**で、想起のクエリ・調停の入力・user メッセージの3つが読む（7箇所）
- **始め方3つを揃えた。** 人の発話（`begin_request`）・情動（`_begin_affect`）・
  機器（`_begin_device`）が、どれも「求めの id を置く → 起点を控える → 手がかりを置く」
  の同じ手順になった。以前は人の発話だけ `_advance_chain` を通らず、同じ場面でも `前進` が
  書かれるかどうかが起点で違った

**完了はテストで見る**（`tests/test_live_record_is_one.py`）。旧名が0件であること、
`KIND_ADVANCE` を書かないこと、3つの始め方が同じ3行を持つこと。
...
### g-ほ は取り下げた（2026-09-08）

④で「(id, 内容) の組が2つある」としたが、**段に で片方（`_chain_head_id`＋
`_chain_head_content`）が消えた**。残るのは次の3つで、**組ですらない**。

| 名前 | 実体 | 寿命 |
|---|---|---|
| `_request_id` | 求めの O の id。求めのあいだ動かない | 求め全体 |
| `_request_text` | 求めの文面。求めのあいだ動かない | 求め全体 |
| `_cue` | いま生きている記録の内容。**版が書かれるたび変わる** | 反復ごと |

`_request_id` と `_request_text` は「求めそのもの」の2つの面で、どちらも動かない。`_cue` は
別のものである。**揃える相手がいないので、この段は要らない。**

### g-へ の実装（2026-09-08・完了）

`_show_seeded` を撤去し、**カーソル自身が「まだ引いていない」を表す**形にした。
...
**挙動が変わった。** DB にやりとりが1件も無いあいだ、毎ターン `latest_exchange_origin()` を
引きに行く（以前は起動から一度きり）。1件でもあれば一度で埋まり、以後は `_close_exchange`
が更新するので引き直しは起きない。**空の DB は初回起動のときだけ**である。

以前の形は、`_recent_cursor` が `None` のまま真偽値が立つと、次に `_close_exchange` が値を
入れるまで直近のやりとりが載らなかった。
...
> v0.7：**全段の実装を反映した**（2026-09-08）。段い〜段へ を実装し、6つの入れ物は
> `Lookup` 1本になった（手で揃える箇所 5 → 0）。`前進`（`KIND_ADVANCE`）と `_show_seeded`
> も撤去した。

> v0.4：**改名の対応表を確定した**（2026-09-08）。

> v0.1：**環-g を新設した**（2026-09-08）。同じ機能に複数の名前が付き、同じ事実が複数の
> 入れ物に入っている状態を、語の側から解く。
