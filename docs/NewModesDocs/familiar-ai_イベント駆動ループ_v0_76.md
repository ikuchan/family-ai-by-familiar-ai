# familiar-ai イベント駆動ループ（#11・段階5）（v0.76）
...
flowchart TD
  ...
  subgraph GATE["出口：delivery gate：出してよいか"]
    ...
    PRES{"居るか：presence：センサあり → 滞留窓内に人（声は数えない）／センサ無し → 在席表 or 声から PRESENCE_VOICE_SEC 以内"}:::proc
...
| 居るか | センサあり：**滞留窓内に人、だけ**（声は視野の外・テレビ・物音でもあるので数えない・v0.76）。センサ無し：在席表 or 声から 60 秒〔仮〕。在席表は「誰か」だけで、センサが誰も居ないを 60 秒〔仮〕見続けたら失効（話者の指定は残す）。カメラの外の人への返事は保留に回り、立ち上がりで配る |
...
> v0.76：居るかはセンサだけ（声を数えない・2026-09-17 17:03 に「こんにちは」の書き起こしへ返事した）。図と表。
