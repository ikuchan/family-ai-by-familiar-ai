# familiar-ai イベント駆動ループ（#11・段階5）（v0.32）
...
段階1 から段階5 まで完了している。残るのは次の2つで、いずれも #11 の外にある。

- **#14**：`see`（撮って見る）と `look`（首を振る）を動作の表へ繋ぐ。既定の反転で新しい系統から落ちており、いまカメラを使う経路が無い。
- **#12**：旧 `run()`・GUI アイドル分岐の撤去（Phase 6）。動体検知の QD への移設も含む。
