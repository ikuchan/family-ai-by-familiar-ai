...
**順3（#15 保留していた発話を配る起点）**である。前提の #8（在/不在）は完了し、実機で入退室が動くことも確認済み（`（なし） → 誰か` と `誰か → （なし）` を各4回観測）。
...
| **3** | **#15** | **保留していた発話を配る起点** | **← 次はここ**（#8 完了で前提が揃った） |
| 4 | **#13** | 深夜の Drive 発火を抑える時間帯倍率（課題10 の `mult`） | 倍率の値の決定 |
| 5 | — | **小さな片付け**（下表） | — |
| 6 | #12 | 旧系統の撤去（Phase 6） | **#14** |
| 7 | #2 | REST 内省（D 加工） | **#8** |
...
| 時間軸の式索引 | `by_time` は `COALESCE(o.last_recalled_at, o.timestamp)` で並べるが、この式に対する索引が無い |
