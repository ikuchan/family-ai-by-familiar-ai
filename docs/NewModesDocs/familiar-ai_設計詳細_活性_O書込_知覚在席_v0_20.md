# familiar-ai 設計詳細：活性・O書込・知覚在席（定数台帳・現状コード所在・移行）（v0.20）
...
|---|---|
| センサあり | **`room_occupied()` だけ**（滞留窓 120 秒内に人）。声も在席表も見ない（v0.20） |
| センサ無し | 在席表に誰か **or** 声から 60 秒以内（従来どおり） |

**声は居るかの証拠にしない**（v0.20・2026-09-17 17:03）：v0.19 では声から 60 秒を補いにしたが、カメラが誰も見ていないのに
「こんにちは」の書き起こしへ返事した。声は視野の外・テレビ・物音でもある。センサがある構成では居るかはセンサだけ、
カメラの外の人への返事は保留（`pending_speech`）に回り、在席の立ち上がりで配られる。声の窓（`PRESENCE_VOICE_SEC`・60 秒）は
センサが無い構成だけで使う。
...
返事（起点が発話）も同じ判定を通る（センサが誰も見ていなければ保留）。判定の全体は `イベント駆動ループ` の
「話す／話さない（To-Be）」の図。
...
> v0.20：**センサがあれば居るかはセンサだけ**（声も見ない・2026-09-17 17:03 実機）。§3-2b。
