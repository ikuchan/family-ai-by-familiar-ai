# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.54）
...
| 知-e | D-rel（関係の加工） | 知-d |
| 環-e | I の口を作り実体を切り出す（OIF → AIF → DIF → 実体） | 依存なし。**挙動を変えない**。OIF と AIF は完了 |
| 環-f | 待ち行列の統合（IIF）と主LLM の非同期化 | 環-e。**挙動が変わる** |
| 環-d | 廃止ストアの移管と撤去 | 記-a・知-e・環-b |
| 記-d | 読み手の居ない列と表の撤去（`scope`・`person_id`・`importance`・`unfinished_business`） | 優先度は低い。**そのうちやる** |
...
| 段 | 内容 | 状態 |
|---|---|---|
| 環-e-い | **OIF を作る**（記憶の追記・読み出し・supersede と**ベクトル埋め込み**を1つの口へ。同期のまま） | **完了**（2026-08-01・公開面8つ・MI 18属性） |
