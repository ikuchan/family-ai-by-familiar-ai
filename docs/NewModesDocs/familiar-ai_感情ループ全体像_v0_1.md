...
- **`T → D`（蓄積）**：`drive_i ← clip(drive_i + rate·mult(t)·learn_i·g_D,i(M)·P_T, 0, 1)`。全欲求共通のレートに、時間帯倍率と学習倍率と気分変調がかかる（発火/mood 別紙 §2.1）。
- **`D → FIRE`（発火）**：`drive_i` が閾値 Θ_fire（≈1）を超えると発火し、放電で drive をほぼ0へ戻す。D が動く経路は「発火時の放電」と「M→D 変調」の2つだけで、I は drive を直接増減しない（§2.4・§2.5）。
- **`FIRE → EVAL → OBS`（感情の生成）**：評価器（軽量LLM）が現ターンの遣り取りを P/Pn/Dom で評価する。A（喚起）は機械値で、値踏みゲート A_gate=0.25 未満は評価器を呼ばず P/Pn/Dom を気分 M のまま置く。評価器は W を入力にとらず、気分をベースに現ターンから感情を出すので、記憶が無くても感情は生まれる。
- **`OBS → W`（想起・1ターン遅れ）**：現ターンの感情は記憶として保存され、**次ターン以降に想起されて初めて** W に入り気分に効く。現ターンの感情が同じターンの気分を動かす直接の経路は無い。
- **`W → NPAD → M`（nudge）**：`N_PAD = (Σ a_i·PAD_i + C·0.5)/(Σ a_i + C)`（C＝自己認識 MI のフラット項の重み2.0）を作り、`A_M ← max(A_M, A_N)`／`X_M ← X_M + A_N(X_N − X_M)`（X＝P,Pn,Dom）で気分を動かす。覚醒が高いほど強く引かれる。
...
| `M → RECALL`（e 軸をスコアへ） | 未接続（スライス3） |

気分 M の最重要の効き先である `g_D(M)`（Drive 変調）が未実装なので、mood-c で M を動かしても、いま live に出る先は評価器のベースだけである。声色・e 軸・Drive 変調が繋がって初めて、気分が振る舞いに滲む。
