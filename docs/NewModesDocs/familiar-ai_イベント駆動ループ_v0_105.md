# familiar-ai イベント駆動ループ（#11・段階5）（v0.105）
...
flowchart TD
  ...
  subgraph T["自律機構：Tonic（T）：常時の背景"]
    ...
    LEFT["失効：mark_absent：在席表から消す"]:::tonic
    SPK{"話者の指定：speaker_known：/speaker 60 秒・返事 60 秒・顔照合のどれか"}:::tonic
    ...
    LIFTQ{"沈黙の期限：silence until：時間切れ・明示の依頼はセンサが誰も居ないを 60 秒見続けたか（タイマー由来は解けない）"}:::tonic
  end
...
| 居るか | **マイクで拾った声は数えない**（テレビ・物音・聞き違い・v0.77）。自分が話してから／`/speaker` を打ってから 60 秒〔仮〕は居る。センサあり：滞留窓 180 秒内に人。センサ無し：在席表。在席表は「誰か」だけで、センサが誰も居ないを 60 秒〔仮〕見続けたら失効。**話者の指定も 60 秒で切れる**（返事するたび延びる・顔照合があれば残る）——切れたら想起は共通の面・名前で呼ばない（知-t） |
...
| 止まったとき（出口） | 情動 → 独白／**退室の知らせ → 独白**（返事する相手は居ない・知-r・v0.80）／機器（入室・タイマー・メモ）→ 保留（`pending_speech`・本文は機器側で決まっている）。発話が起点の求めは入口で止まるので出口には来ない |
