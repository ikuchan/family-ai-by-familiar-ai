# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.37）
...
| S4 | 見えの普通（DINOv2 の定点別 EMA）→ $\widehat{S}_{景色}$ | #8 |
| S5 | 見回り | #16 |
