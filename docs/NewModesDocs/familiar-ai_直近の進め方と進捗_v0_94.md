...
**生きているもの**（未決・課題8 の表に無いか、表の行の補足）。解消したものは下の「解消済み（記録）」へ畳む。
...
- **GUI に渡した発話が画面に出なかった（2026-09-12 実機・未解決）。** 「何が見えますか？」
  ...
  **関係する直し**（09-15・出-s）：人が話しかける前の自発の発話が画面に出なかった件は、GUI が
  `start_autonomy()` の直後に出口を結んでいなかったのが原因で直した。09-12 の件（GUI のターン終了後に
  駆動体から来た返事）と同じ根かは**未確認**——次に起きたら上の 3 行で見る。
...
### 解消済み（記録・2026-09-17 に畳んだ）

- **本番 DB へ未適用のマイグレーション 3 本（057・059・060）**——本番の `schema_migrations` で 057（09-05）・059・060（09-07）を確認、065 まで適用済み（2026-09-17 読み取り）。PAD の測り直しは 記-g で完了（09-13）。
- **CUI の入力欠落**（日本語入力中に 10 秒ごとの再描画で未確定文字が落ちる）——原因の見当は付いたが**対応は見送り**と決めた。
- **`person_memory_manager.py` の静的解析 8 件**——2026-09-17 の `mypy` で 0 件。解消済み。
- **現行 `run()` の想起件数を `recall_k` へ**——旧 `run()` は 環-c で撤去。対象そのものが無い。
- **P-2（参照申告で $n$ 増減）・MI 集約段・p 軸**——申告で根づきを動かす口は実装済み（`apply_verdicts`）、$n$ の減りは 記-a-ろ-ろ（09-15）、MI の集約は層 1（畳む・固める）に置き換え。p 軸の実挙動は 課題8 F-3（知-e）。
- **調停の時間切れ**——5.0 秒に決めて Config へ（`根拠台帳` §19）。層 3 の登録値でもある。
- **STT の利用枠**——TTS に続き STT もローカル化（2026-07-30）。`STT_ENGINE=elevenlabs` へ戻せる。
- **話していないのに書き起こされた**（2026-07-30・幻聴）——`no_speech_prob` で分かれ（幻聴 ≥0.722・本物 ≤0.709）、`STT_NO_SPEECH_MAX`（既定 0.72）で落とす。
- **実機で未確認のもの**（07〜08 の一覧：つなぎの口調・保留の解放・「分からない」の出口・打ち切り…）——**何が未確認かの正本は `課題8` §7 の束 C** に移した。
- **1 つの発言に 2 回答えた**（2026-07-30・GUI）——確定した書き起こしを二人が積んでいた。`_committed_relay` は表示用だけにして塞いだ。
- **見回りが実機で成立しなかった**（2026-08-01・`look` が選ばれない）——見た印を鎖の外へ出し（`direction="観察"`）、`look` は調停の候補に載り（09-16・知-m）名前と相対の向きで回った。**見回り（自発）が定点を巡ること**は 課題8 E-1（知-c）。
- **`see` が見えたものを言葉にできていなかった**（2026-08-01）——VLM 返答のコードフェンスでパースが落ちていた。直した（知-a）。
- **全体テストの並列度**——bge-m3 がワーカーごとに載る（GPU 1.7〜2.3GiB）ので既定 `-n 4`（`RUN_TESTS_PARALLEL` で変えられる。パジュが動いている機体では `-n 1`）。
- **同じモジュールを提供する配布物が 2 つ**（2026-07-30・リポジトリ外・SBV2 の 500）——宣言どおりの配布物に揃えて解消。
- **`mcp-server-fetch` の依存を固定**（2026-07-29・リポジトリ外）——`mcp` 2.0.0 の改名に追随していないため `uvx` の版を固定した。
- **`getattr` 形は `grep` に出ない**——撤去の前は `getattr(self, "_x"` も検索する（環-c で `_concerns` を誤って消した教訓）。
...
> v0.94：**保留・申し送りを「生きている」と「解消済み（記録）」に分けた**（2026-09-17）——26 項目のうち 16 を 1 行ずつに畳んだ（未適用マイグレーション 3 本は本番で適用済みを確認・`run()` の想起件数は対象が無い・P-2 と MI 集約段は層 1 に置き換え・`look` が選ばれない件は知-m で解け見回りは E-1 へ・静的解析 8 件は 0 件 ほか）。生きているのは 10（GUI の吹き出し・l10n・感情の tuning・`self_model`・周囲の会話・11 分の一式・`FAMILIAR_ENV_FILE`・`~/.familiar-ai.json` の `python`・検索 MCP の空行・配信ゲート）。
