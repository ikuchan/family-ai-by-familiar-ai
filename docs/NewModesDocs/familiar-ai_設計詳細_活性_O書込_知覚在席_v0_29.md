# familiar-ai 設計詳細：活性・O書込・知覚在席（定数台帳・現状コード所在・移行）（v0.29）
...
| **声の名乗り**（知-w・v0.26・2026-09-19）——**在席（カメラ）があるときだけ** | 「パパだよ」を調停が `speaker_claim` に読み、機械が在席と家族の名前を検めて `/speaker` と同じ効き（`set_active`・`_speaker_set_at`・PMM 同期）。在席が無ければ名乗りは入口で飲まれたまま（マイクは在席の証拠にしない・変えない）。家族に無い名前は付けない。道具の帰りの反復では読まない |
