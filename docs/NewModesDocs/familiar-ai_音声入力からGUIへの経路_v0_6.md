# familiar-ai 音声入力から GUI への経路 v0.6
...
**名前を手がかりとして渡す**（v0.6・2026-09-17）。「パジュ」は日本語の語彙に無く、実機で
体重／パチュー／はじゅ と書き起こされ、「パジュ、静かにして」の沈黙依頼が名前の守りで 3 回とも
落ちた（15:31〜15:32）。`create_realtime_stt_session` が `ME.md` の「名前：」の**先頭 1 語**を
`STTConfig.hotwords` に入れ、`_transcribe` が faster-whisper の `hotwords` に渡す。先頭だけに
するのは、`ME.md` に並べた聞き違いの綴り（はじゅ）まで渡すと STT をそちらへ寄せるためで、
沈黙の守り（`core/silence_rules.names_me`）のほうは全部の名前をゆるい読みで見る
（`イベント駆動ループ` v0.74）。効果は実機で測る（`課題8` 束 C）。
...
| （`ME.md` の「名前：」） | — | 先頭 1 語が `STTConfig.hotwords` として書き起こしの手がかりになる。env からは与えない |
