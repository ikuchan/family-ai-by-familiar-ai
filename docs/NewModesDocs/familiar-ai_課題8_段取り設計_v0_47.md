# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.47）
...
**記-a（REST 内省）**である。起動と骨格は完了しており、次は蒸留にあたる。環-c（旧経路そのものの撤去）も完了した。環-b は実機確認で欠陥が出て 知-d 待ちになり、情-b と小さな片付けは完了している。
...
| 溜まっているもの | 状態 |
|---|---|
| **知-c 見回りの実機確認** | 実装は済んでいる（着手順から外れて先に実装した）。**首が実際に回ることを一度も確かめていない** |
| **調停の時間切れ** | 「黙って」のとき **4.18 秒**かかると実測。時間切れ 2.0 秒では届かない。**値の決定待ち** |
| **STT の利用枠** | ElevenLabs の無料枠を使い切った（10,000/10,000・2026-07-29 10:47 に回復）。**音声を使う実機確認は、それまでできない** |
...
| 課題 | 項目 | 待っているもの |
|---|---|---|
| **記-a** | **REST 内省**（D 加工） | **← いまここ**（起動と骨格は完了・次は蒸留） |
| 知-c | 昼夜の見回り（ユースケース③） | 実装済み。**首が実際に回ることを一度も確かめていない** |
| **出-a** | 音声出力のローカル化 | **TTS は完了**（2026-07-30・実機確認済み）。**残るは STT** |
| 記-b | D-mi（MI 畳み込み・蒸留） | 記-a |
| 知-d | 顔と声の登録 | 登録に応じられる人が揃うこと（外部条件） |
| 環-b | 保留していた発話を配る起点 | 知-d（起点は動くが、配ったものが人に届かない） |
| 知-e | D-rel（関係の加工） | 知-d |
| 環-d | 廃止ストアの移管と撤去 | 記-a・知-e・環-b |
| — | p 軸の実挙動・大人と子どもの口調・一定時間で忘れる在席 | 知-e |

**知-c を先に置いたのは、実装済みで確認だけが残っているためである。** 動かなければ実装が無駄になったまま気づけない。カメラが実機で動くことは環-c の確認で見ている。

**出-a に依存は無い。** ここに置いたのは他が機構で音声出力が仕上げという並びのためで、先に必要になれば前へ動かせる（**実際、記-a の途中で前へ動かした**）。
...
**残るは STT である。** `faster-whisper` の依存は既に入っているが未使用で、`stt.py`（録音して起こす）と `realtime_stt.py`（常時集音）の両方が ElevenLabs のままである。常時集音には VAD が要り、**マイク（Yamaha YVC-300）が HVAD・エコーキャンセラ・ノイズリダクションを内蔵している**ので、音量ベースの単純な VAD で足りるかを先に計測して決める（silero-vad を入れるかはその後）。**HVAD の判定結果はホストへ渡されない**ので、音声を自分で見る必要がある。
