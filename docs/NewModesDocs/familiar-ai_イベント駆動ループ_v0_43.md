# familiar-ai イベント駆動ループ（#11・段階5）（v0.43）
...
**see の帰りは調停を飛ばして主LLM へ戻す**（v0.43）。見ると決めたのは主LLM 自身で、画像を受け取るのも主LLM だけである。軽量LLM に判定し直させると `light` を選んで画像を見ずに即席のラベル文だけで答えたり、`recall` へ逸れたりした（実機）。`_intake` が see の完了を取り込んだ反復は、`_decide` が調停を呼ばず `full` を返す。思考の深さは主LLM が `see` を出したときの値（`Request.see_effort`）を引き継ぐ。調停の約 1 秒も消える。
