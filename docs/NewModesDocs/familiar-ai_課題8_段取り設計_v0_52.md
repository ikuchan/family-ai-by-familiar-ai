# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.52）
...
| 知-e | D-rel（関係の加工） | 知-d |
| 環-e | I の口を作り実体を切り出す（OIF → DIF → 生成器・動作器・想起） | 依存なし。**挙動を変えない** |
...
| 段 | 内容 |
|---|---|
| 環-e-い | **OIF を作る**（記憶の追記・読み出し・supersede と**ベクトル埋め込み**を1つの口へ。同期のまま） |
| 環-e-ろ | **DIF を作る**（身体・MCP-音楽・MCP-検索・主LLM への呼び出しを1つの口へ。中は同期の転送） |
| 環-e-は | **実体を切り出す**（生成器・動作器・想起を `event_loop.py` から出す） |

OIF を先にするのは依存が最も少ないためである。記憶は呼ばれる側であって、他のコンポーネントを呼ばない。
