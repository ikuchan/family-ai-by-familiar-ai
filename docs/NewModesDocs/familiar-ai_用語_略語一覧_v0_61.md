...
| IIF | 内部接続 | Internal Interface | IIF | 非同期のやりとりが通る I の口。きっかけと結果が並ぶ**1本の待ち行列**を持つ。AIF・DIF と内部コンポーネントは、すべてここを介してやりとりする（`設計図` ③-2・2026-08-01 決定）。③ の3つのキュー（AIF／DIF／完了）はここへ統合する。**未実装。** |
...
| **完了キューの種別** | 完了キュー（QC）が運ぶものの種類。**3つ**——`完了`（調べものの結果が届いた）・`進捗`（まだかかっている。結果ではないので飛行中の数も版も触らず、短い一言のためだけに反復を起こす）・**`決定`**（主LLM の返り。`TurnResult` と、投げたときに見ていた W を `Decision` として運ぶ） | 「決定」を調停（軽量LLM）の選択の意味で使わない——そちらは `branch` である |
