...
次の一歩はスライス3、すなわち e 軸を想起スコアへ入れることである。e は加算部の一項なので、現行の純積（`cosine × time_score × _derive_activation(a0,n)`）からハイブリッド合成への切り替えが要る。乗算で足すと e が拒否権になり設計と逆になるためである。ハイブリッドの r 伸長が前提としていた平均中心化は C2 で済み、伸長の係数 c_lo=0.0・c_hi=1.0 も決まったので、スライス3 を塞ぐものはない。

残りのスライスは次のとおり塞がっている。

- P-2（参照申告で n 増減）は新ループ（課題8・[D-反復出力]）待ちで保留。案a が依存する主LLM↔ツールループ自体を新ループが作り直すため、その上で実装する。
- P-4（5軸合成）は p 軸を待つ。r・t・a・e の4軸はスライス3 で揃う。
...
実機確認は P-1〜P-4 と W2b-2 をまとめてスライス3 の後、知覚の前に行う。シナリオは実機確認シナリオ文書へ溜めてある。
