# familiar-ai イベント駆動ループ（#11・段階3）（v0.20）
...
**調停には人格（ME）を渡す。** 発話の出口は2つ（調停の light・つなぎ と、フルLLM の答え）で、軽量側にだけ ME が無いと、同じ人格が2つの口で違う口調で喋る（実機で「調べてくるね！」と「調べてみますね。」が混ざった）。
