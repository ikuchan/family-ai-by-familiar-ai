# familiar-ai ユースケース⑤：朝の自発ニュース（新フレーム）（v0.1）
...
## 現状コードとの関係（【コード事実】）
- 起動時に記憶を読み直して自伝の橋（昨日→今日）を架ける仕組み（旧 `_morning_reconstruction`）は**撤去した**。実行中の経路から呼ばれておらず、同じことは MI の想起で実現する方針である。本ケースの想起は「**朝の最初の活動での R の recall**」として扱う。
...
sequenceDiagram
  ...
  O->>O: 「ニュースを話した」・開いた意図の 根づき を落とす（解決）
...
9. **B 閉じる**：「ニュースを話した」を O に記録。開いた意図の 根づき を落とす（解決・supersede は使わない）。
## 注意点
