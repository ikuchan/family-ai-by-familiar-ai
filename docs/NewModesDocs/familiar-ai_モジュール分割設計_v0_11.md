...
I（イベント駆動）
   ...
   │ DIF（機器接続：身体＝カメラ・マイク・スピーカー／MCP-音楽／MCP-検索／主LLM）  ← 非同期
   │ OIF（記憶接続：追記・読み出し・supersede・ベクトル埋め込み）                ← 同期
O（記憶ストア・追記と supersede）   W は O からの派生ビュー
SS（自己状態）   Config（全調整可能定数）
```

**同期にするのは OIF と ULM だけである。** どちらも「その反復の中で結論が要る」処理で、待ち行列へ回すと1反復で結論が出せない。主LLM は DIF の向こうにあり非同期である。
...
io/       出入口（`設計図` ③-2 の4つの口）
  ...
  dif.py            機器接続（DIF・身体／MCP-音楽／MCP-検索／主LLM）
...
| **環-e-は** | **DIF を作る**。身体（カメラ・マイク・スピーカー）・MCP-音楽・MCP-検索・主LLM への呼び出しを1つの口へ集める | `io/dif.py` | 不変（中は同期の転送） |
...
DIF はその次で、外部への呼び出しを1つの口へ寄せるだけなら中身の順序は変わらない。実体の切り出しは、口が揃ってからのほうが移す先が決まっている。
