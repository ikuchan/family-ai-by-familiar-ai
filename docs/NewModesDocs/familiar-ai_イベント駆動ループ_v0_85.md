# familiar-ai イベント駆動ループ（#11・段階5）（v0.85）
...
**解ける条件を居るかの層で見る**（情-l・v0.82・2026-09-18）。「頼んだ人が居なくなれば解ける」は在席表（「誰か」を言うだけの層・失効で消える・知-p）で見ていたため、タイマー中に在席表が失効（`/speaker` から 60 秒）した瞬間に解け、「こんにちは」に返事した（実機 14:51）。改めた：**明示の依頼**はセンサが「誰も居ない」を 60 秒〔仮〕見続けたとき（`agent.nobody_since`・T の在席の刻みが持つ）だけ解ける（センサが人を見ている・無いなら解けない）。**タイマー由来**（`reason=timer:`）は在席では解けず、鳴る・止めるまで。期限・本人の「話していいよ」・止めては変えない。`is_silenced(req, now, nobody_since)`——在席表の名前照合は撤去。
