# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.34）
...
| #8 | **在席と身元**（旧 #7 顔登録 S ＋ 旧 #8 在席系の精緻化を統合） | 未着手（後回し集合・**内訳は下記**） |
| #9 | 音声出力 | 未着手（後回し集合） |
...
**旧 #7（顔登録 S）と旧 #8（在席系の精緻化）を1つにまとめた。** 分けていた理由は「登録が済まないと身元解決の精度を確かめられない」という依存だったが、実際には**同じ二層（在/不在と誰か）を作る作業の表と裏**である。登録の入口が無ければ身元は自己申告に頼るしかなく、身元の器が無ければ登録しても置き場所がない。別々の課題として順に置くと、片方だけ進めて確かめられない状態が続く。

用語一覧が定める**二層**（在/不在＝T(G)・連続、誰か＝I・必要時）に対し、実装は在席と身元を1つの関数に混ぜている。解く対象を具体化しておく。

**土台（旧 #7）**

- **登録の入口（S）**：顔と声の登録を GUI／CLI から行えるようにする。認識の載せ替え自体は実装済み（InsightFace(ArcFace) 顔＋ECAPA-TDNN 声・GPU 実行・`presence_watcher` が `person_arrived`／`person_left` へ結線）で、**残るは登録の入口と実際の登録**（Phase 3 の記述と同じ）。
- **p 軸（在席者相関）の実挙動**：スコアラの結線は実装済みで、**在席入力が埋まればそのまま効く**。登録が済むまで実挙動を確かめられない。
...
- **人に合わせた発話の実機確認が未了**：`ME.md` は丁寧さを相手で決める（大人にはですます、子どもには打ち解けて、分からなければ丁寧に）と定め、話者は `_present_ctx` 経由でプロンプトへ渡り、`/speaker` はイベントループ経路でも効くようになった。**ただし、相手を変えて口調が実際に変わることは実機で確認できていない**。在席と身元を整理したあと、大人と子どもの双方で確認する。

**在席の判断が定まれば、ここで一緒に片付くもの**

在席は発話の可否・宛先・想起の重みに効くので、いま別々に見えている小さな症状のいくつかは、この課題の中で解ける。**別の課題として立てず、ここで扱う。**

- **退室が実機で起こせない**：`presence_watcher` はカメラが要り、`PersonTool`（`note_person_arrived`／`note_person_left`）はエージェントに登録されていない（`grep` で0件）。QD の退室イベントは実装とテストがあるが**実機で一度も通っていない**。登録の入口（S）かツールの登録で通る。
- **保留していた発話を配る起点**：在席がゼロから立ち上がった瞬間だけ配る。いまその瞬間を作れるのは `/speaker` だけで、**在席が正しく上下しないと配れない**。
- **一定時間で忘れる在席**：MI の想起に駆動される在席（活性の派生ビュー）は、記憶が人に紐づくこと（D-rel）に依存する。二層を分けるときに、どちらの層が寿命を持つかを決める。
- **会話経由の入退場**：`PersonTool` を登録すれば LLM が「◯◯が来た／帰った」を記録できる。カメラが無くても在席が動く経路になり、**退室の実機確認にも使える**。
