...
- 同時に **5 本〔仮〕**まで。調停の候補には載せない（full で処理・つなぎ不要）。道具は `_ACTIONS`／`_FULL_ACTIONS`／`_LOOKUP_ACTIONS` に載り、`recall` と同じく結果はその場で返って次の反復が言葉にする。見出しは入力ごとに別（「タイマーを掛ける「パスタ」」）で、同じ求めで掛けて止めるができる。
