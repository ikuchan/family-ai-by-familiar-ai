...
### 記憶は出来事でなく「面」に付く（2026-09-01 記録）

**1つの出来事は、関係の面ごとに複数の記憶になる。** 面は `(obs_id, person_id, relation_key)`
で、`actor`（誰がやったか）／`addressee`（誰に向けられたか）／`about`（誰についてか）／
`present`（誰が居たか）などがある（初期集合は3つと定めたが、実物は9種へ育っている・
[D-在席相関/V2]）。

**面ごとに分ける理由は、畳んでも「誰が何を言ったか」を再構成できるようにするためである。**
出来事を1行だけで持つと、supersede で畳んだ瞬間にその情報は畳んだ側の `content` の文字列に
...
| 量 | 何ごとか | 置き場所 |
|---|---|---|
| `content` | **面ごと**（その関係の角度から書いた言葉） | 面 |
| 取込の驚き `a0` | **出来事ごと**（パジュにとっての驚き。1回だけ測る） | 出来事 |
| 時間の起点 | **面ごと**（パジュがその面を最後に引いたのはいつか） | 面 |
| 根づきの `n` | **面ごと**（パジュがその面を大事だと言った回数） | 面 |

「人ごと」ではない。`person_id` は所有者でなく**誰と関係する面か**を表す。
...
| `beneficiary` | `[自分のためにされた]` | 同上 |
| `companion` | `[一緒にした]` | 同上 |
| `source` | `[自分から伝えた]` | 同上 |
| `owner` | `[自分のものについての]` | 同上 |

**面の生成は二段である。** 段①＝機械（`actor`・`present`）が全観測に土台を立て、
段②＝REST 内省が本文を読んで意味役割を抽出し、**既存の観測にもさかのぼって**足す。
`actor` が content を持たないのは、全観測に立つので書き直す意味がないためである。

**`default` は視点ではない。** 書き込み側で「話者がまだ分からない」を表す置き場であって、
人ではない。面は実在の人と `__self__` にしか立たないので、想起の視点は `__self__` へ寄せる
（`store.context.viewpoint_of`）。**誰が書いたか（`writer_id`）と、誰の視点で引くかは別の
問いである。**
### 想起は面ごとに返す（2026-09-02 実装）

`DISTINCT ON (o.id)` で1観測1候補へ畳んでいたのをやめた。同じ出来事に2つの関わり方で
触れたことは、畳んで消してよい情報ではない。**想起の n 件は「面 n 件」の意味**になる。

畳むのをやめても件数は膨らまない。**想起は必ず1つの視点で引く**ので（`WHERE s.person_id
= %s`）、視点を固定すると同じ観測の面はほとんど並ばない。同じ出来事の複数の面は、多くが
別々の人に付くためである（`actor` はパジュ、`present` は家族）。

| 視点 | 面 | 観測 | 観測あたりの面 |
|---|---|---|---|
| `__self__` | 2811 | 2672 | **1.05** |
| 家族A | 471 | 412 | 1.14 |
| 家族B | 160 | 142 | 1.13 |
| 家族C・D | 62・42 | 62・42 | 1.00 |

（本番 2026-08-21・生存する観測のみ）

**面の言葉が初めて読まれるようになった。** `[役割の札] ` 付きの content は 2149 行に
入っていたが、想起は `o.content`（出来事の本文）だけを返していたので 1 箇所も読まれて
いなかった。面が言葉を持たないとき（`actor`）は出来事の本文を出す。
...
> でどの面で当たったかを捨てていた。`id` を面（`situated_memories.id`）にし、`obs_id`・
> `person_id`・`relation_key` を持たせ、視点3属性（`writer_id`／`subject_id`／
> `participants`）を落とした。**表は分けたまま**（`superseded_by` が出来事の列にしか
> ないことが「畳んでも面は残る」を保証しているため）。想起は面ごとに返すが、1視点あたり
> 同じ観測の面は 1.00〜1.14 枚しか並ばない。

> v0.11：**面の `content` の規則（`[役割の札] ` ＋ 出来事の本文）と、生成が二段で
> あることを §5 へ記録した**（2026-09-02・047＋048）。段①＝機械（`actor`・`present`）、
> 段②＝REST 内省が意味役割を既存にもさかのぼって足す。`default` は視点でないことも記した。

> v0.10：**記憶が出来事でなく「面」に付くことと、その理由を §5 へ記録した**（2026-09-01）。
> 1つの出来事は関係の面ごとに複数の記憶になり、`content`・時間の起点・根づきの `n` は面に、
> 取込の驚き `a0` は出来事に付く。分ける理由は、supersede で畳んでも「誰が何を言ったか」を
> 再構成できるようにするため（求めの版チェーンの動作で必要になった）。主体はパジュ一人で、
> `person_id` は所有者でなく誰と関係する面かを表す。
