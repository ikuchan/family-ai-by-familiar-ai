# familiar-ai ユースケース③：昼夜見回り（新フレーム）（v0.2）
 
**相対の向き（2026-09-16・知-m ③）**：`look` は `pose`（定点名）のほか `direction`（右／左／上／下）を受け、いまの向き（`CameraTool.position()`）から**その方向で最も近い定点へ絶対移動**する（`poses.pose_toward`）。名前の無い向きへは行かない（設計は変わらない）。右＝pan **負**（`RIGHT_IS_POSITIVE_PAN=False`・実機 2026-09-16 12:41 で仮置きの正が逆と確認）。向いたあとは**動き終わるまで待ってから撮る**（`CameraTool._settle`・読んだ位置が目標の 0.02 以内に収まるまで 0.2 秒刻み・上限 3 秒。実機 16:50 で 0.4 秒固定の待ちでは途中の位置で撮っていた）。その方向に定点が無ければ「知っている場所」を挙げて動かない。
...
- 次の定点は **`[いま]` の「見ていない順」（`PresenceMap.stale_order`・定点ごとに最後に見た時刻）**から LLM が選び、書かなければ機械が最も長く見ていない定点を入れる（v0.2・2026-09-17）。v0.1 の「O→W 構築で最も薄れた印」は、印が無い定点が W に浮かず「見ないから印が無く、印が無いから見に行かない」になった（2026-08-01）ので置き換えた
- 見回りの完結ループは**実装済み・実機で未確認**（`課題8` E-1）
  （2026-09-10 確認）。`look` は動作の表に載り（`_ACTIONS`）、`_run_camera` が首を振り、
  見た印を鎖の外へ独立した記録として書く。定点は `poses.py`、定点別の在席は
  `presence_map.py`、「普通」は `pose_norms.py` が持つ。**ただし首が実際に回ることを一度も
  確かめていない**（`課題8` の 知-c＝「いまここ」）。当初の記述（下）は当時のものである。部品（絶対 pan/tilt・Per-pose-norm・見た印 MI の 根づき 減衰・配信ゲート）を新フレームに組み直す。
## シーケンス図（I の反復版）
...
> v0.2：次の定点は `[いま]` の「見ていない順」＋機械の既定（2026-09-17）。想起頼みをやめた。
