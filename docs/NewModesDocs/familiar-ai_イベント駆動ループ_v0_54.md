# familiar-ai イベント駆動ループ（#11・段階5）（v0.54）
...
**MCP の同期の道具は、動作名と道具名の両方で引ける**（v0.54・知-j・2026-09-13）。`house_rules` は動作の表に載っていたが、主LLM が `get_house_rules` を呼んでも `_LOOKUP_ACTIONS` に道具名が無く捨てられ、調停が投げても `_dif.lookup`（検索と取得しか知らない）へ流れて空振りしていた——実機で一度も呼ばれた記録が無い。`_MCP_LOOKUPS`（動作名／道具名 → 道具名・見出し）を置き、`_run_lookup_body` が `DIF.call_tool` で呼んで完了として積む（`recall` と同じ）。`family_schedule`（`get_family_schedule`・家の予定・`設計方針_家の予定との接続`）を足し、調停の候補にも繋がっているときだけ載せる（`extra_actions`）。
