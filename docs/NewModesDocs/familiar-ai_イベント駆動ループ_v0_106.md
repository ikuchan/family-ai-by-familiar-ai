# familiar-ai イベント駆動ループ（#11・段階5）（v0.106）
...
上限に達した反復では、フルLLM に「**調べきりたかったが上限に達したことを述べ、そのうえで現時点で分かることを返す**」と指示する。黙って手持ちで繕うと、材料不足だったことが相手に伝わらない。あわせて**「した」と言える基準**を渡す（v0.88・出-z・実機 2026-09-18 18:31）：「「した」と言えるのは、道具の返りにそう書いてあるときだけ。返りに無いことは、していない。できていないなら、できていないと言う」。上限で答えさせた主LLM が、W に「まだ掛けていない」と並ぶ中で「1 分のタイマーかけたよ」と言った。嘘のつもりは無く文脈から補ったので、「嘘をつかない」では止まらない。基準を渡す。
...
drive 発火でも身体の動作を取るが、**発話は聞く相手が居て初めて意味を持つ**（正本③ の配信ゲート＝結果有り＋在席）。v0.106（出-as）からは、在席で止めるのは情動の求めだけで、会話の求めはウェイクワードの窓で決める。止められた発話は保留せず、独り言として O に残す（以前の `pending_speech` への保留と、人が来たら配る仕組みは出-as 段 9b で外した）。

### 話す／話さない（v0.106・出-as・2026-09-26）

入口（黙る・窓）→ 反復 → 出口（情動の軸・窓・在席）→ 結末 を 1 枚に。決まりの正本は `設計方針_話していいかの決まり` v0.3、「居るか」の詳細は `知覚在席` §3-2b、タイマーの音は `設計方針_タイマー` v0.2。
...
flowchart TD
  ...
  subgraph T["自律機構：Tonic（T）：常時の背景"]
    ...
    SENS["在/不在の層：PresenceSensor：YOLO・定点別・滞留窓"]:::tonic
    DRV["欲求の蓄積：accumulate：bond・esteem は人が映っているときだけ溜まり、映っていなければ 1/3 の速さで減る"]:::tonic
    DIFF["人の出入り：scan_presence：入室・退室を記録だけする（求めを立てない）"]:::tonic
    SPK{"話者の指定：speaker_known：/speaker 60 秒・返事 60 秒・顔照合・写真の見立てが 1 人"}:::tonic
    UNK["分からない：clear_speaker：共通の面・名前で呼ばない"]:::tonic
    NUDGE["押し上げ：nudge seeking：窓の外の声がして誰も見えない → SEEKING に発火閾値の半分"]:::tonic
    LIFTQ{"沈黙の期限：silence until：時間切れか（タイマー由来は鳴るまで・人が居なくなっても解けない）"}:::tonic
  end

  subgraph IN_["入口：intake：きっかけを求めにするか"]
    TRG["きっかけ：Trigger：会話入力（声／キーボード）／機器／情動／完了"]:::io
    SREQ{"沈黙依頼：silence_request：いま黙っているか（明示 or reason=timer:id）"}:::proc
    LIFT{"解くきっかけ：lifts：機器「タイマー」／名前と「話していいよ」が同じ発話（誰でも）／タイマー由来ならその操作の言葉"}:::proc
    HOLD["溜め：silence_hold：会話は原文・機器と情動は件数・O には残す"]:::mem
    ONE["まとめ：one request：聞けるようになった最初の求めの W に「黙っていたあいだに届いたもの」"]:::mem
    CTRL{"タイマーの操作：control word：タイマーが動いている／鳴っているときの「止めて」など"}:::proc
    WIN{"窓：wake window：キーボードか・名前を聞いたか・窓が開いているか"}:::proc
    DROP["捨てる：discard：記録しない（ログだけ）"]:::out
  end
  ...
  subgraph GATE["出口：delivery gate：出してよいか"]
    AXIS{"情動の軸：fired axis：情動なら bond・esteem か"}:::proc
    WCL{"窓切れ：window closed：会話の求めで窓が切れているか"}:::proc
    PASS{"通り抜け：passes_gate：確かめて掛けたタイマーの鳴りか"}:::proc
    ORIG{"起点：origin：会話か"}:::proc
    PRES{"居るか：presence：センサあり → 人が映っているか／センサ無し → 在席表"}:::proc
    FILL{"つなぎ：said_fillers：もう一言出した相手が居るか"}:::proc
  end

  subgraph OUT_["結末：outcome"]
    SPEAK["話す：DIF.speak：声と吹き出し・会話なら窓を 1 分へ延ばす"]:::out
    QUIET["沈黙：silent：声にしなかっただけの独り言として O に残す"]:::store
    MONO["独白：monologue：O に「〇〇に言いたかったこと：…」・根づき 2"]:::store
  end
  ...
  SREQ -- "黙っている" --> LIFT
  LIFT -- "違う" --> HOLD
  LIFT -- "解くきっかけ（会話なら窓を開ける）" --> ONE
  HOLD -. "解けたとき" .-> ONE
  ONE --> ARB
  SREQ -- "黙っていない・会話入力" --> CTRL
  SREQ -- "黙っていない・機器／情動" --> ARB
  CTRL -- "はい（窓を開ける）" --> ARB
  CTRL -- "いいえ" --> WIN
  WIN -- "受ける（開ける／延ばす）" --> ARB
  WIN -- "窓の外の声" --> DROP
  ...
  TXT -- "空" --> QUIET
  TXT -- "有る" --> AXIS
  AXIS -- "seeking・safety・rest" --> QUIET
  AXIS -- "bond・esteem、または情動でない" --> WCL
  WCL -- "切れている" --> MONO
  WCL -- "開いている、または会話でない" --> PASS
  PASS -- "はい" --> SPEAK
  PASS -- "いいえ" --> ORIG
  ORIG -- "会話" --> SPEAK
  ORIG -- "情動・機器" --> PRES
...
| 場面 | 決まり |
|---|---|
| 黙っている | 入口で止める（情-h）。窓の中で頼まれたら話者に関わらず受け、既定 30 分（最大 60 分）。解くのは機器「タイマー」・名前と「話していいよ」が同じ発話にあるとき（誰でも）・時間切れ。タイマー由来の沈黙はその操作の言葉を通すが、言葉では解かない。人が居なくなっても解けない |
| 会話入力 | キーボードはいつでも受けて窓（1 分）を開ける。声は `ME.md` の名前が文のどこかにあれば（1 字違いまで）開けて受け、窓が開いていれば延ばして受ける。どちらでもなければ捨てる。名前が設定されていなければ声は何も聞かない。タイマーが動いている／鳴っているときの操作の言葉は名前が無くても通し、窓を開ける |
| 居るか | センサあり：人が映っているか（滞留窓）。センサ無し：在席表。**マイクで拾った声は数えない**（v0.77）。自分が話してから／`/speaker` から 60 秒は居る、は外した（出-as 段 9a） |
| 話者 | `/speaker`・返事・顔照合のほか、写真の見立てが 1 人で名前の分からない人も居なければその人を話者にする。60 秒で「分からない」に戻す（知-t） |
| 出口 | 情動は bond・esteem のときだけ声にし、人が映っていなければ独白。会話は窓の中なら在席に関わらず声にし、窓が切れた後の返事は独白。機器（タイマー・アラーム）は在席で止め、確かめて掛けたタイマーの鳴りは通り抜ける |
| 独白 | 相手の面に「〇〇に言いたかったこと：…」（相手が分からなければ自分の面に「誰かに言いたかったこと：…」）を根づき 2 で書く。声に出した返事で使ったら「伝えた：…」を書いて元の記録を畳み、根づきを 0 に戻す |
| 知らせ | 人が来た・居なくなった・メモは記録だけ（求めを立てない）。黙るのが明けても何もしない |
| 静穏時間 | 発話の門は無い。夜に話しかけないことは、欲求の溜まり方（倍率 0.083）が持つ |
| つなぎ | 窓が切れていれば言わない。言ったら窓を延ばす。一言出した相手には本応答も出す（黙っていての依頼だけは守る） |
...
`said`（言ったつなぎ）は手組みのまま残す。O にもあるが、この求めで言った分だけを載せる（配る保留の `held` は v0.106 で外した）。

一覧は投げるたびに記録している `(語 → 動作)` から組み、求めが決着した時点で捨てる（`_finish`）。合成した状態ラベルではなく、**実際に投げたものの記録**である。

### QD（DIFキュー＝機器）と人の出入り（スライス2）

> v0.106（出-as 段 6）で、人の出入りは求めを立てずに記録だけする（`DIF.record`）ようにした。QD に積むのはタイマー・アラームの鳴りである。以下は当時の記録。
...
（v0.106・出-as 段 9b で外した）**保留していた発話を配るのは、在席がゼロから立ち上がった瞬間だけ。** 保留は「聞く相手が居なかった」から溜まったものなので、相手が現れた瞬間に配れば足りる。会話中に家族が増えるたび割り込ませない。同時に2人来ても配るのは1回。寿命（鮮度切れ）は `pending_speech` 側が持つので、新しいキューは作らない。
...
**（2026-09-25 追記）この前提は実機と合っていない。** 退避ログの主LLM は `effort=low` が **235 回・中央 5.33 秒**・p90 7.67 秒、`medium` が 73 回・中央 6.57 秒だった。**`low` でも 5 秒以上黙っている**ことになる。枝（`effort`）で「遅そうか」を当てるやり方は外れている。直すのは出-aq の続き（Jev を入れたら見直す）で、ここでは事実だけを記す。
...
### つなぎの作り直し（出-aq・2026-09-25・暫定・Jev を入れたら見直す）

**つなぎの目的は 1 つである。** 「時間のかかる作業に AI が入っているときに、考え中・調べ中であり、
**無視しているわけではない**ことを伝える」（本人）。あわせて「同じ言葉を何度も言うのは機械的すぎて
不可」「言ったつなぎを無視して主LLM が話し始めるのも避けたい」。

**つなぎの声は待たない。** `_say_filler` は再生の終わりまで待ってから戻っており、**その後の調べもの
と主LLM が待たされていた**——実機 2026-09-24 15:58、検索はつなぎが鳴り終わるまで **4.59 秒**待ち、
画面の吹き出しも声の後だった。いまは声だけを背景で鳴らし（`_speak_filler_in_background`）、画面・
記録・O への書き込みはすぐやる。声が本応答と重ならないのは `TTSTool` の鍵（一度に一つしか鳴らさない）
が保証する。終わるときは声を待たずに止める。

**つなぎは `say` とは別の道具（`filler`）にする。** 主LLM は、つなぎを知らなかった——W は調停の前に
文字列で組まれ（`workspace_ctx`）、つなぎはその後に出る。つなぎを主LLM に渡す形を実機 15:49 の条件で
測った（`根拠台帳` §48）。

| 渡し方 | 本題を `say` で言った | 挨拶を繰り返した |
|---|---|---|
| 渡さない（それまでの実機） | 8/8 | 8/8 |
| **`say` を使った発言**として会話に置く | **1/8**（7 回は「もう答えた」で黙った） | 1/8 |
| **別の道具**を使った発言として置く | **10/10** | 8〜10 割 |

`say` は返事なので、つなぎを `say` で表すと主LLM は「`(応答は既に送信済み)`」と書いて黙った。
**つなぎと返事を同じ道具で表したので、見分けがつかなかった。** 別の道具にすれば、主LLM は「`say` は
まだ使っていない」と道具の区別で知る。

**主LLM の会話に、自分の発言として置く。** 主LLM を呼ぶ 1 箇所（`_run_main_llm`）で、言ったつなぎを
「`filler` を使った発言＋その返り」として会話に足す。通常の呼び出しも整合チェックの差し戻しも、
ここを通る。**返りは「言った」の報告だけで、指示を入れない**——返りに「本題はまだ」と書くのは報告の
口の流用である（本人の指摘）。つなぎが何であるかは、道具の定義（`FILLER_TOOL`）が持つ。定義は
**規則ではなく、つなぎが何であるか**を書く（返事の前半・`say` はその続き・重複は機械が消す）。
「〜するな」は、システム文で 9 通り・道具の定義で 6 通り試して、どれも繰り返しを減らさなかった。

**道具を使った発言を一から組む口を、6 つの担い手すべてに足した**（`make_tool_call_message()`・
本人「全部に足して」）。道具の返りには共通の口があったが、道具を使った側の発言は API の生の返りを
そのまま使う作りで、一から組めなかった。形は 4 通りある。

| 担い手 | 形 |
|---|---|
| Anthropic | `content` に `tool_use` |
| Gemini | `parts` に `function_call`（＋検証を飛ばす署名） |
| OpenAI 互換（道具を API で渡す）・Kimi・GLM | `tool_calls` に `type: function` |
| OpenAI 互換（道具を文で書かせる）・CLI | 本文に `<tool_call>{…}</tool_call>` |

**Gemini 3 系は、作った呼び出しを拒んだ**（`400 Function call is missing a thought_signature`）。
形の試験は通っていたのに、実物は会話の中の道具の呼び出しに思考の署名を要求する。Google の文書が
定める、作った呼び出しのために検証を飛ばす値（`skip_thought_signature_validator`）を入れて通した。
同じ文書は「作った呼び出しを差し込むのは、避けられないとき以外は強く勧めない」とも書く。
OpenAI 互換・Kimi・GLM・CLI は鍵が無く、形だけ確かめた。

**主LLM が自分で `filler` を使ったとき**：調べものと一緒ならつなぎとして鳴らし（`say`＋調べものも
いままでどおりつなぎに回す）、`say` と一緒なら先に鳴らし、**つなぎだけなら返事として出す**（黙らせ
ない）。

**繰り返しは機械が落とす**（`core/filler_echo.drop_echo`・出-aj #4）。本応答の冒頭がつなぎと重なっ
たら、その一節を落とす。挨拶の一覧は持たない。最初の一節が 2 字未満（「あ、」）なら次の一節を照らし、
相槌は残す（段 3）。「機械が重複を消す」と道具の定義で伝えると、繰り返しそのものは減らないが、
主LLM は**落とした後に自然になる形**（「こんにちは！**あ、そういえば**…」）で書いた。

**調停の `filler` 欄と、W の「すでに相手へ伝えた一言」枠は残す。** 調停が 2 回目以降のつなぎを
言い換えるのに使っている（同じ言葉を繰り返さない）。
...
**語の軸を足し、軸ごとの順位で底上げする（記-k・2026-09-22）。** 埋め込みは文全体の似かたを見るので、問いの形（「覚えてる？」）に引かれ、**中身の語**では引けない。実機 15:58、5 分前に自分が話した「…キャンプの準備…本を更新…」は、時間の軸では 32 位で候補に入っていたのに、採点では 11 位で 7 件に届かず「思い出せない」と答えた。**取りこぼしは候補集めではなく採点にあった。**

直しは 2 つである。(1) `ObservationStore.by_words`（問いの名詞を含む記録を新しい順に $N$ 件）を union へ足す。語は `pyopenjtalk` の名詞から細分が 一般・固有名詞・サ変接続 のものだけを採り（「さっき・前・いま」＝副詞可能 は時を指すだけなので外す）、`word_counts` で数えて母数の 25% 以上に当たる語は落とす。(2) 各軸での順位を**畳まずに**持ち、$\sum 1/(k+順位)$（$k=10$）を適合度に足してから、床と件数で絞る。いくつもの軸で上位に来た記録ほどきつく持ち上がる。**適合度の目盛りは残る**ので、5 軸の効きは変わらない。

**足した値は 1.0 で頭打ちにする。** 軸の本数は在席者の人数で増えるので底上げに固定の上限が無く、そのまま足すと適合度が 0〜1 に収まらなくなる。頭打ちが効くのは類似がほぼ 1.0 の記録だけで（実機の最大は 0.389）、並びは変わらない。

本番の記録 11,571 面で測った結果は次のとおり。探していた記録は **11 位 → 1 位**（軸は 時間 32 位・語 1 位、底上げ 0.108）。すでに働いている 3 つの問い（「フーコックの宿、決まった？」「コーチングどうなった？」「押入と金木犀ってどう読む？」）では、7 件のうち 4〜6 件がそのまま残り、入れ替わったものはいずれも話題に合っていた。情動軸を入れても、中立の気分でもそのときの気分でも 1 位で変わらない。
...
> v0.106（出-as 段 6）で静穏時間の発話の門を外した。夜に話しかけないことは欲求の溜まり方（倍率 0.083）が持つ。以下は当時の記録。
...
> v0.106（出-as 段 5・6）で変えた：黙る依頼は窓の中で受け（名前の関門は窓にまとめた）、既定 30 分。解くのは名前と「話していいよ」が同じ発話にあるときだけ（誰でも）。人が居なくなっても解けず、明けても求めを立てない。誰も見えなかったあいだの控えも外した。以下は当時の記録で、いまの決まりは「話す／話さない（v0.106）」の節と `設計方針_話していいかの決まり` v0.3 にある。
...
**「沈黙が明けた」を起こすのは、黙っていたあいだの分があるときだけ**（情-k・v0.80・2026-09-18）。器（`_muted`）は不在で聞いたもの（理由「誰も見えなかった」・知-p）とも共有しており、`check_silence_lifted` が件数だけを見ていたため、誰も見えないのに「沈黙が明けた」の求めが立って LLM が回り返事が保留に溜まった（実機 12:38）。理由が「黙っていた」のものが 1 件も無ければ起こさない。不在の分は人が映った求めに載る（本来の作り）。

**情動の求めで調停が「黙る」（light・text 空）と決めたら、主LLM を起こさず沈黙で閉じる**（出-w・v0.81・2026-09-18）。候補文は「text を空にすれば黙る」と言っているのに `_parse` が判定不能として None を返し、フルへ倒れて主LLM が無駄に回っていた（実機 12:28）。発話が起点の light 空は従来どおり判定不能（返事しないのは決めていない）。

**解ける条件を居るかの層で見る**（情-l・v0.82・2026-09-18）。「頼んだ人が居なくなれば解ける」は在席表（「誰か」を言うだけの層・失効で消える・知-p）で見ていたため、タイマー中に在席表が失効（`/speaker` から 60 秒）した瞬間に解け、「こんにちは」に返事した（実機 14:51）。改めた：**明示の依頼**はセンサが「誰も居ない」を 60 秒〔仮〕見続けたとき（`agent.nobody_since`・T の在席の刻みが持つ）だけ解ける（センサが人を見ている・無いなら解けない）。**タイマー由来**（`reason=timer:`）は在席では解けず、鳴る・止めるまで。期限・本人の「話していいよ」・止めては変えない。`is_silenced(req, now, nobody_since)`——在席表の名前照合は撤去。 **解けたら記録も消す**（v0.86・情-l-ろ・実機 20:46）：明示の依頼が退室で解けても記録が `until` まで残り、タイマーの沈黙（`hush_for_timer`）が「人の依頼が生きている」と負け、人が映ればまた黙った。`check_silence_lifted`（T の tick）が、明示の依頼で `is_silenced` が偽なら `clear_silence()` を呼ぶ（器が空でも）。タイマー由来は鳴る・止めるまで消さない。
...
### いま道具から返ったものは W の最上部（出-x・v0.83・2026-09-18）

道具の返り（`set_timer` の「確かめて」・検索の結果）は版の全文として `[過去の記憶]` の列に載るだけで、調停はそれを記憶として読み、**いま届いた返り**として扱わなかった——「確かめて」に聞き返さず `set_timer` を掛け直し、4 反復空回りした（実機 14:50）。取込がその反復で受けた返り（`Request.just_returned`）を `[いま道具から返った]` として W の最上部に 1 行ずつ載せ、W を組んだら空にする（次の反復には持ち越さない・版の全文は残る）。実験（`根拠台帳` §35）：これだけで 8/8 が light の確認文。先導文の差し替えは 6/8 で `action` を誘発するので入れない。

### タイマー・アラームの帰りの反復は想起なし（出-x・v0.84・2026-09-18）

上の枠だけでは足りなかった。同じ W の再試行では 8/8 でも、実機（15:28・15:42）では W が少し違い、調停はまた `set_timer` を選んだ。実機ログから拾った 3 種の W で比べると（`根拠台帳` §35）、いまの形は 24 回中 22 回が `action`。原因は想起の列である。`set_timer` の帰りの手がかりは「…の結果が届いた：まだ掛けていない…」で、想起は同じ道具の過去の帰り（「掛けた」「すでに調べた」）を 5〜6 件並べ、調停はそれを「いまの頼み」として読む。

帰りの反復では、道具（タイマー・アラーム・確認への答え）から返ったときに限り**想起を回さない**（`workspace.RETURN_WITHOUT_RECALL`・`Workspace.returned_actions`）。W は `[いま道具から返った]`（上の節）と直近のやりとりだけで組む。直近は残す。聞き返しに失敗した記録が消えると、何度失敗したかが分からなくなる。検索・見る・思い出すの帰りは従来どおり想起を回す。

調停の先導文は道具の返りを受けるもの（`arbiter._LEAD_TOOL_RETURN`・`arbitrate(tool_return=True)`）に替える。返りが人に伝える文ならそのまま light、確かめてなら聞き返すだけで掛け直しはしない、直近に「うまくできなかった」があっても取り返そうとして道具を選ばない、の 3 点を書く。末尾の一文が無いと、失敗の記録がある W では 8 回中 2 回しか light にならなかった。文は実験のものをそのまま正本にする（実験と本番で文が違うと裏付けが消える）。あわせて、返ってきた道具そのものは候補から外す（`_extra_actions(exclude=)`・`cancel_timer` などは残す）。この 3 つで 3 種の W とも 8/8 が light の確認文。

もう 1 つ、機械の守りを外す（v0.85・出-x-ろ・実機 18:31）。`arbitrate` は light の返りに「道具が要る頼み（`needs_tools`・「測って」など）は light で答えない → full」の守りを掛ける。帰りの反復でも発話は「…測って」のままなので、この守りが確認文の light を full へ倒し、主LLM が掛け直して上限まで空回りした。`tool_return=True` の反復では守りを掛けない（道具はもう使った）。初回の反復の守りは残す。実験は守りの後で数え直した（`根拠台帳` §35）。

調停は人の**名乗り**も読む（v0.89・知-w・2026-09-19・`Decision.speaker_claim`）。機械（`_apply_speaker_claim`・`_apply_silence` と同じ呼び口 `_apply_requests`）が在席（カメラ）と家族の名前を検め、`/speaker` と同じ効きで話者を付ける。在席が無い発話は入口で飲まれるので、名乗りは自然に使われない（`知覚在席` §3-2b）。

帰りの反復では**沈黙の依頼も読まない**（v0.86・情-n・実機 20:46）。発話は古く（「パジュ、3 分測って」・名前入り）、返りの文に「黙って」が入る（確認文「その間は黙って待機します」）ので、調停がこれを人の「黙って」の依頼として読み、タイマーが掛かる前に 60 分の沈黙を掛けた（「いいよ」が飲まれた）。`arbitrate` は `tool_return` なら `silence_minutes`・`lift_silence` を機械で 0／false に落とす。黙るかどうかはタイマーの道具（`TIMER_SILENCE`）が決める。

逆に、タイマーが動いている／一時停止中に届いた**操作の言葉**（再開・一時停止・止めて・`timer_rules.is_control_word`）を調停が light で受け流したら full へ倒す（v0.86・出-aa・実機 20:47「はい、分かりました。再開しますね」と言うだけで `resume_timer` は呼ばれなかった）。`needs_tools` と同じ型の守りで、`_decide` が `[タイマー]` の枠の有無（`timer_active`）を渡す。言葉はあいまいでありうるので機械で道具を選ばず、主LLM が枠と道具で決める。帰りの反復には掛けない。

帰りの先導文には「した」と言える基準も入れる（v0.90・出-ag・実機 09-19 13:39）：ストップウォッチ 2 本目の断り「いま「…」を測っている。止めてから」が返ったのに、調停が「測り始めますね」と言葉だけ OK した。「「した」と言えるのは、道具の返りにそう書いてあるときだけ。返りが断りなら、その断りをそのまま伝える」（出-z と同じ基準・実機の W で 8/8 が断りを伝えた）。あわせて、誰か分からない在席の注記は「名前で呼ばない（直近のやりとりの名前も当てにしない）。知りたければ聞いてよい」（出-ae(1)・13:14「パパ、おかえりなさい」）。

**思い出し方を変える 4 つの道具**（v0.93・出-ah・2026-09-21）。W の「過去の記憶」に何が載るかを決める軸は 10 個ある（手がかりの言葉・視点・件数 K・候補数 N・床・5 軸の重み・時期の基準と幅・1 件の字数・直近の窓・根づき）。このうち主LLM が動かせるのは**手がかりの言葉だけ**（`recall(query)`）で、時期を移せるのは調停（`time_ref`）、残りは設定値だった。思い出せないとき、7 件・いまの相手の面・いま基準・直近 5 分という条件そのものを変える手が無い。そこで軸ごとに道具を分けて渡す。

**道具の一覧に置くだけでは使われない（出-ah-ろ・2026-09-22）。** 4 つを渡した翌日の実機で、「9 月初旬どんな話してた？」「日記じゃなくてパジュの記憶を探って」「探し方を変えてもう一度探して」の 3 回とも、1 度も使われなかった。ログで確かめたところ、**主LLM は 3 回とも起きており**（反復 3／2／3）、道具も渡っていた。使われなかっただけである。引き方の 1 行は毎回 `7 件まで（11 件）` と出ており、想起は満席に見えていた。

実機の作業状態と本物の道具定義で、主LLM（`claude-sonnet-5`）が何を選ぶかを数えた。**言葉の側の手当ては、単独ではどれも効かない**——引き方の一行に手応えを足す・規則を義務にする・返りに手応えを書く・規則を手順にする・引き方をプロンプトへ写す・正直の規則に思い出す努力を足す・記憶の申告の規則に足す、の 7 通りがいずれも 8 回中 0〜1 回だった。`say` を道具から落とすと 8 回中 7〜8 回になる。

効いたのは 2 つの**組み合わせ**である。(1) 身体の定義に `:id memory` を足し、**記憶を自分の視点で引く器官**として名乗らせる（目・首・声・net は器官として書かれているのに、記憶を引くことだけが道具の一覧にしか無かった）。(2) `[返事]` の行に「返事を書くのは、引き直せることをやり切ってから」を足す。**片方だけでは 16 回中 3 回、両方で 40 回中 26 回**。実装した形（規則も許しから手順へ改めた）では 24 回中 21 回になった。

規則 `widen-before-giving-up` は「引き方を変えてよい」から「『思い出せない』と言うのは引き直した後だけ。まだ一度も引き直していないなら say を呼ばずに先に呼ぶ」へ改めた。整合チェックへは渡さない（文だけでは違反を判定できない）。

| 道具 | 動かす軸 | 上限 |
|---|---|---|
| `recall_as` | 視点（誰の面から引くか） | 家族の誰でも。**返りの先頭に「（パパの面で思い出した）」を必ず付ける**——記憶の空間は人ごとに独立しており、別の人の記憶を目の前の相手に話す経路になるため |
| `recall_deeper` | 件数と 5 軸の重み | 件数 1〜20（既定 20・候補数 50 が天井）。思い出し方は人の言葉で指す（「新しい順に」＝時間／「印象に残っていることを」＝情動／「よく思い出すことを」＝根づき／「この人との関わりで」＝視点／「話に近いものを」＝関連）。指した軸を **2 倍**・他を **2/3**。**床（0.05）は触らせない** |
| `recall_when` | 時期の基準と幅 | ISO の日付（`2026-08-15`）＋幅（既定 30 日）。言葉を日付に直すのは主LLM の仕事 |
| `recall_recent` | 直近の窓 | **30 分・20 往復**まで |

引くのは既存の口（`workspace.recall`・`Workspace.recent_text`）で、新しい引き方は作らない。**効き目はその 1 回だけ**で、求めには何も残さない（次の反復の W は元の条件で組む）。上限は既存のものだけ——投げるたびに 1 反復と 1 回の考えを消費するので、反復 5・考えた回数 5 に収まる。返りは `recall` と同じ行に、**何を広げたか**と**母数**（「候補 50 件から N 件・載せられるのは K 件まで」）を添える。丸めるだけで断らない（上限を超えた指定は上限に丸める）。

**足りないことに気づく手がかりも要る**（同上）。道具があっても、W に何件・どの条件で引いたかが書かれていなければ、主LLM は「足りない」と気づけない。記憶の列の先頭に 1 行置く（`workspace.describe_basis`）：`[この想起：いまの相手の面・7 件まで（3 件）・いま基準・直近 5 分／6 往復]`。**人の名前は出さない**（誰か分からないときに名前で呼ぶ材料になる）。規則にも 1 条足した（`widen-before-giving-up`「思い出せないときは、諦める前に引き方を変えてよい」）。整合チェックへ渡す 7 つには入れない——文だけでは違反を判定できない。

**過去の記憶も全文で載せる**（v0.92・2026-09-20）。以前は 1 件 120 字で切り、細部は直近のやりとりの枠（逐語）が担うとしていた。その直近を 3／6 往復・5 分に狭めたので（出-ae(2)）、5 分より前の話は 120 字の断片しかどこにも残らなくなった。量は枠（`workspace_max_chars`＝40,000 字）が受ける——枠の計算はもともと全文で行っており、載る件数は `recall_k`（7 件）で決まるので、切り詰めは枠のためには要らない。切ると中身が落ちることは実機でも起きていた（検索結果の『9月14日(月) 30℃/22℃』が 120 字の外に出て、調停は検索し直し、主LLM は「読み取れなかった」と答えた・2026-09-13）。W に入らなかった件数は INFO に残る。

**直近のやりとりの窓には時間の上限もある**（v0.91・出-ae(2)・2026-09-19）。記-h の枠は往復数（3／6）だけで切っていたので、再起動をまたいで 1 時間半前（11:37）の「パパ、…」が 13:14 の最上部に 2 行並び、調停はそれを「いまの相手」の手がかりにした。`MemoryConfig.recent_exchanges_max_sec`（**300 秒**・本人の決定・層 3 は動かさない）より古い行は、起点でも継起の鎖でさかのぼった行でも直近には載せない（`render_recent` の `max_age_sec`）。外した行は直近の `exclude` から外れるので、点が付けば過去の記憶の列に出る。外したときは INFO 1 行（`直近: 300 秒より古い N 行を外した`）。

不在の門にも同じ規則を置く（v0.87・出-ab・実機 22:13）。鳴った瞬間にタイマーの沈黙は解け、代わりに「誰も見えない」の門（知-p）が会話入力を飲むので、映らない位置からの「止めて」が届かなかった。`[タイマー]` の枠があるか音が鳴っているとき（`_timer_active`）、操作の言葉（`is_control_word`）は誰も見えなくても通す。タイマーが無ければ今までどおり飲む。

### 掛ける前の確認は機械の状態（出-y・v0.84・2026-09-18）

「確かめて」の返りを聞き返せても、「いい」の後で LLM が `confirmed=true` を付けて呼び直す約束は残っていた。主LLM は聞く前に `confirmed=true` を付けて掛けた（実機 15:42・id=13）。LLM に掛け直しの判断を渡す限り、この穴は閉じない。

確認は求めでなく**機械の状態**が持つ（`core/confirm_state.py`・`agent._pending_confirm`・`設計方針_タイマー` §10）。道具は確認が要ると判定したら預かり（道具・入力・聞いた時刻・聞いた文）を置き、確認文だけを返す。`confirmed` は道具の定義に無く、LLM が入力に書いても読まない。次の求めでは、預かりが生きているあいだ（`CONFIRM_TTL_SEC`・300 秒〔仮〕）W の最上部に `[確認待ち] タイマーを掛ける「パスタ」（3 分）：「…いい？」` が載り（`Request.confirm_frame`・求めを始めるときに写す）、調停の候補と主LLM の道具に `confirm`／`decline` が載る。`confirm` は機械が預かった入力で道具を呼ぶ（`call(..., confirmed=True)`・キーワード引数）。`decline` は捨てる。どちらも道具の帰りとして次の反復に届く（想起なし・上の節）。別の `set_timer` が来れば預かりは上書きされる（「5 分にして」）。

### 確認待ちのあいだに「掛けた」と言わせない（出-ag-ろ・2026-09-21／09-25）

実機 2026-09-21 17:34：「3分測って」→ `set_timer` は確認待ちを返した（掛かっていない）。調停が同じ求めのなかで
`set_timer` を 3 回選び、上限の反復で `light` を選んで「**タイマーをセットしました**」と言った。確かめの問いは一度も
人に届かず、預かりだけが残った。入っているのは次の 4 つである。

1. **確認待ちのあいだは、掛ける道具を入口で落とす**（`core/confirm_state.blocks`・09-21・`2b75f71`）。預かりが
   生きているあいだの `set_timer`／`set_alarm`／`start_stopwatch` は実行せず、「いま『…』でいいか確かめている。
   先に答えてもらう」を返す。答える（`confirm`／`decline`）・止める・話す・調べるは通す。
2. **確認の問いは機械が読む**（`_ask_confirm_if_fresh` → `_ask_confirm_aloud`・同）。道具が新しい預かりを
   置いたら、その場で枠の文をそのまま声に出す。LLM に作らせない（本人の決定）。あわせて規則
   `no-claim-while-confirming`（確認待ちのあいだは「掛けた」と言わない）を発話前の検査へ渡す。
3. **同じ語を二度投げないときは、前に返ったものをそのまま返す**（`_dispatch_lookup`・09-25・`b328d2d`）。
   以前は「結果は W にある」と返していたが、タイマーの返りの反復は想起をしないので W に無かった。
   まだ返っていなければ「（まだ返っていない）」と書く。
4. **道具名を書き換えた後にも、候補に照らす**（`arbiter._parse`・09-25・`4a54d46`）。読み取りは候補に照らした
   **後で**道具名を書き換えていた（`set_alarm` に何分後を書くと `set_timer`）。返りの反復で外した `set_timer` が
   そこから戻ってきた。書き換えた先が候補に無ければ、直接書いたときと同じく読めない選択として倒す。

**枠は求めの始めにだけ写す**（これまでどおり）。反復ごとに写す直し（`9e70024`）は、実物の調停が枠を見て
**人の返事なしに `confirm` を選んだ**（16/16・`根拠台帳` §49）ので打ち消した（`f6d9e5e`）。検査へ
確認待ちの行を渡す直し（`8f4d4c9`）も、行が無くても捕まえていた（8/8）ので打ち消した（`548ce83`）。
調停の `light` の文は、これまでどおり発話前の検査を通さない（本人の判断）。

**「整合チェック」は「発話前の検査」（`speech_check`）へ改名した**（09-25・`6f3de83`・`55a6225`）。何と何の
整合かが名前から分からなかった。中身は変えていない。**これより前の節は、書いた時点の名前（整合チェック・
`coherence`）のまま残す**——この文書は段階ごとの実装記録で、当時のログの文と照らせるようにするため。
...
3. **過去の記憶**——想起（5 軸）で上がったもの（**全文**・v0.92・2026-09-20）。1 に載った記録は id で除く。
...
> v0.106（出-as 段 9b）で保留を積む口と配る口を外した。止められた発話は独り言（「〇〇に言いたかったこと：…」）として O に残り、想起で上がる。以下は当時の記録。
...
（v0.106・出-as 段 9a：直近の発話で居るとみなす分岐と、自分が話してから 60 秒の窓は外した。居るかはカメラで決める。）**配信ゲートは「誰かがいる」で決める**（v0.51・知-h・2026-09-13）。「聞く相手が居ない」の判定（`agent._social_presence_permission`）を、在/不在の層（YOLO）→ 顔の照合 → 直近 5 分の発話、の順にした。顔が未登録でも目の前に人が居れば話す。詳細は `知覚在席` §3-2b。
...
- 保留していた発話を配る起点（在席がゼロから立ち上がる瞬間）が来ない（この起点は v0.106 で外した）
...
> v0.106（出-as 段 5）で、名前の関門は窓にまとめた。窓は名前で開くので、窓の中の「黙って」は名前が無くても受ける。既定は 30 分、上限は 60 分。
...
**繋がっていない道具は、黙って消えていた（出-al・2026-09-22）。** `DIF.tool_defs()` は MCP が無いと空を返し、その動作は調停の選択肢からも主LLM の道具からも消える。エラーにも記録にもならない。失敗として記録されるのは「投げた道具が失敗を返した」ときだけ（`failed_actions`）で、**存在しない道具は投げられない**のでそこを通らない。実機 17:50、カレンダーを設定から外した状態で「明日の予定わかる？」と聞くと、3 回続けて「調べたけど出てこない」と言い、道具が無いことには一度も触れなかった。予定を作り話はしていない。

直しは 2 つである。

**(1) 能力を、いま取れる道具で数える。** 能力の一覧の門は `enabled_env: MCP_CONFIG` だったが、この機体は `MCP_CONFIG` を設定せず既定パス `~/.familiar-ai.json` を使うので、**MCP 由来の能力は動いていても一覧から落ちていた**（実態と逆のずれ）。門を `enabled_tool: get_family_schedule` へ改め、**その道具がいま取れるときだけ**有効と数える。REST 内省の層 4 は、材料に「いま実際に取れる道具の名前」（`## Live tools`）を足したうえで一覧と要約を書き直す。設定ファイルに書いてあることと、繋がっていることは別である。

**(2) その求めの時点で、使えない道具を 1 行で見せる。** `[いま使えない] 家族の予定を見る道具（カレンダー）` を `[返事]` の手前に置く。層 4 は誰も居ない晩にしか回らないので、昼に落ちた場合はこの行が埋める。

実機の作業状態と、カレンダー抜きの道具一式で測った。**1 行を置くだけで 8 回中 8 回が伝える。** 置かないと 0 回で、8 回とも `search_notion` を選び直した（実機の症状そのもの）。「聞かれたら正直に言う」という言い聞かせを足しても 8 回のままで、**文は要らない**。

## 写真から推し量った身元を、在席へ流す

**推し量ることは禁じない。推し量ったなら機械にも使う**（本人の決定・2026-09-22）。実機 15:57、押入と正面を見た直後に、調停が写真を見て「パパ、おかえりなさい！」と言った。ところが機械は `(present :speaker "unconfirmed")` のままで、在席の注記（「名前で呼ばない」）も渡っていた。以後ずっと食い違ったまま「パパ」と呼び続けた（出-ae-は）。

**出したのは主LLM ではなく調停である。** `see` を調停自身が出したときは、その帰りを調停が**写真つきで**判断する（`調停が出した see の帰りなので調停が判断する（写真つき）`）。その発話は整合チェックを通らない——検査が走るのは主LLM の `say` の返りだけである。

身元が機械に入る道は 2 本あった。顔（`PersonMemoryManager.presence_status`）と、声の名乗り（調停の `speaker_claim` → `_apply_speaker_claim`・知-w）。**写真からの見立てだけが、どこへも行かなかった。**

直しは 3 つである。

**(1) 調停の返りに `seen_people` を足す。** `[{"name": "パパ", "confidence": 0.8}]` の形で、写真に写っていた人を書かせる。**誰か分からない人は name を空にして数に入れる。** 名乗り（実際に言われた言葉）とは別の欄にする——証拠の強さが違う。実機の写真と作業状態で測ると、欄を足せば 5 回中 5 回書く（足さなければ `speaker_claim` は空のまま）。ただし**確信度は 1.0 と書いてくる**ので、機械側で 0.6 を上限に掛ける（`core/seen_people.SEEN_CONFIDENCE_MAX`）。

**(2) 見立てを在席へ足す。** `_apply_seen_people()` が、家族に当たった人を `person_arrived` で在席へ入れる。**足すだけで誰も消さない**——消すのは顔の消失と時間切れである（声からの打ち消しは 出-am）。在席表が空だったときは、その人が話者にもなる（`person_arrived` の既存の振る舞い・本人の決定）。見立てが空の反復では在席を触らない。

**(3) 名前の分からない在席者を持てるようにする。** 在席表に入れられるのは登録済みの人だけで、「居るが誰か分からない人」を表せなかった（残課題 #8）。`note_unknown_present(count)` が札（`unknown:1`）で人数ぶんを持ち、`presence_status()` には `name="不明"`・`person_id=None` で出る。**`get_present_ids()` からは外す**——あれは観測の `participants` になって人ごとの面を立てる材料なので、混ぜると誰にも対応しない記憶空間に面ができる。見立ては求めごとに言い直されるので、呼ばれるたびに足すのではなく**いまの人数に合わせる**。

あわせて 2 つの穴を塞いだ。**注記が落ちる穴**——「名前で呼ばない」の注記は在席表が**空**の経路にしか無かったので、名前の無い在席者を持てるようにしたとたん、表が空でなくなって注記が落ちる。名前の分からない在席者しか居ないときは注記を付ける。**名乗りが当たらない穴**——`resolve_claim` は呼び方の一覧まるごととしか比べておらず、「パパだよ」「ママだよ」では話者が付かなかった（当たるのは `名前` の欄と一致する言い方だけ）。呼び方を一つずつ照合し、呼びかけに使う名前（先頭）を返すよう直した。

**主LLM も同じ道を通す（出-an・2026-09-22）。** 写真は主LLM にも届くので、同じ穴が残っていた。実機 15:57 の写真で測ると、**6 回とも「あ、パパだ！おかえりなさい!」と呼び、機械へは何も渡らない**。1 回は「眼鏡をかけた人が見えたよ、パパみたいだね」と、`FAMILY.md` の外見から推し量ったことを自分で述べていた。

`say()` の入力に `seen_people` を足す（`memory_verdicts` と同じ形で、**発話と同時に**申告させる）。欄を足すと **6 回とも書く**（確信度 0.7〜0.9・調停の 1.0 固定より控えめ）。道具として渡す案（`declare_speaker`）は採らなかった——道具を投げた反復は発話を持たないので、名前を呼ぶたびに一拍遅れる。`see` の帰りはすでに反復を 1 つ使っている。

受け口は調停と同じ `_apply_seen_people()` で、確信度の上限も同じ 0.6 に揃える。どちらも写真からの推し量りで、証拠の種類は同じだからである。なお `tools/person.py` の `PersonTool`（`declare_speaker` ほか）は、いまも**どこからも生成されていない**死んだコードのままである。

**引き方の 1 行は、採点ぶんと関連ぶんを分けて数える（出-ai・2026-09-22）。** 以前は `7 件まで（11 件）` と並べており、「上限 7 なのに 11 載っている」と読めた。**11 はすべて W に載っている**——採点で採った 7 件に、関連想起（拡散・`diffuse_max_add`＝4）が足した 4 件が加わった数で、`Workspace.build` は件数を切らない。課題に書いた見立て（「W に載るのは枠に入るぶんだけ」）は事実と違った。いまは `似ている順に 7 件・関連で 4 件` と書く。分けるのは、引き直す道具が効くのが**採点ぶんのほう**だからである。由来は `Recalled.by_association` が持つ（`retrieval_method == "diffuse"`）。

## 身元を打ち消す道（出る口）

**在席へ入る口は 3 つあるのに、出る口は時間切れと顔の消失しか無かった（出-am・2026-09-22）。**

| 消えるもの | きっかけ | 場所 |
|---|---|---|
| 在席表ぜんぶ | センサが「誰も居ない」を 60 秒見続けた | `loop/tonic.py`（`mark_absent`） |
| 話者の指定 | 60 秒返事が無く、顔も見ていない | `loop/tonic.py`（`clear_speaker`） |
| その人の在席 | 顔の信号が 120 秒途切れた | `recognition/presence_watcher.py`（`person_left`） |

声からの道はどれにも無く、**誤った見立ては寿命が来るまで残る**。そこで調停の返りに `not_person` を足す。人が「自分はいま呼ばれている名前の人ではない」と打ち消したら、否定された呼び方をここに書かせる。名前を挙げない打ち消しなら、いま話者としている人の呼び方を書く。

受け側（`_apply_not_person`）は、家族に照合してから `person_left()` で在席から外し、話者を戻す。**由来は問わない**（本人の決定）——顔で入った人でも、目の前の人が打ち消せば外す。**否定は名乗りより先に処理する**。逆だと「ちがう、ママだよ」で、付けたばかりの ママ を否定で消すことになる。

実測では、「パパじゃないよ」「ちがうよ」がどちらも 3 回中 3 回で読まれ、名乗り（「ぼく、たいきだよ」）と普通の返事（「うん、ただいま」）では 3 回中 0 回（誤りなし）だった。**見本を全部外すと、名前を言わない打ち消しが 3 回中 0 回に落ちる。** 番人（`test_prompt_holds_no_quotable_sample_utterances`）が許す範囲——「〜」で始まる形か 3 字以内——の見本を置いて 3 回中 3 回に戻した。

あわせて、出-ae-は で作った穴を塞いだ。「誰も居ない」60 秒の失効は `get_present_ids()` を回しており、**名前の分からない在席者（札）を外したせいで通らなくなっていた**。失効は在席表そのものを畳む仕事なので、札まで含めた `present_keys()` を回す。

**名乗りによる打ち消しは範囲外**（本人の決定）。「ぼく、たいきだよ」は名乗りとしてだけ読まれ、話者は たいき に変わるが、見立てで入れた パパ は在席に残る。

## 口調は、いま向き合っている相手で決まる

**規則は届いていて、守られなかった（出-ak・2026-09-23）。** 実機 17:25、`/speaker パパ` で話者が確定している状態で「…分からないんだ。ごめんね、パパ！」と常体だった。`ME.md` には「大人（パパ・ママ）には〜です〜ます」とあり、`FAMILY.md` にも「大人」とあり、どちらもシステム文に入っていた。

実機の作業状態で 8 通り測ると、**単独ではどれも効かない**——規則（`personality-from-me`）の書き換え 0/12、在席の行に「この相手は大人」0/12、`[返事]` の行に口調 2/12、W の過去の発話に相手を添える 1/12。効いたのは**3 つ揃えたときだけ**で 11/12 になった。

引っぱっていたのは **W に並んだ自分の過去の口調**である。`[直近のやりとり]` の行は `わたし：自分が答えた：…` としか書かず、**誰へ言ったかが落ちていた**。想起は相手を問わず過去の発話を引くので、子どもへの常体（`えー、ひみつなの!? 気になるなあ`）が大人との会話の W に並ぶ。

直しは 3 つ。

1. **W の行に相手を添える**（`わたし（パパへ）：…`）。そばに居た人（`present` の面）から自分を除いたもの。**面が立っていなければ添えない**（相手を捏造しない）。
2. **在席の行に `:note "この相手は大人"`**。`FAMILY.md` の「関係」で判じる。書かれていない人には添えない（決めつけない）。
3. **`[返事]` の行に `ME.md` の話し方の行をそのまま置く**。機械は**写しを持たない**——`ME.md` を「大人にも常体」に書き換えれば、そのとおり常体になる（実測）。

抜いた行の言い回しが効き目を決める。`〜を基本にする` では 6/12、`〜で話す` では 11/12 だった。**含みのある言い方は弱い。**

主体と相手は**1 つの口**（`ObservationMemory.voices_of` → `OIF.voices`）で一度に取る。W の 1 行が両方を要るためで、別々の面にすると口が 2 つになり問い合わせも 2 回になる。

## つなぎを、つなぎに戻す／宛先の条件

**つなぎが用件を言い切っていた（出-aj・2026-09-23）。** 実機 15:49、「こんにちは」に対して 144 字が出た——「…どなたでしょうか？ あ、それから、お伝えしたいことがいくつかあります。メモを更新した内容、覚えました。フーコック旅行は…」。**本応答は 35 字で混ぜていない。混ぜたのはつなぎのほうである。**

根は、同じ `text` という欄を 3 つの用途が共有していたことである——`light` では**それが返事**、`full`／`action` では**つなぎ**。`light` の書き方が既定になっていた。文言では止まらない（3 通り測って 6/6 で混ぜる）。**欄を分け、言いたいことの行き先（`trash`）を置くと 0/6 になる。** 削って測ると、要るのはこの 2 つだけで、「空にしない」と JSON の並び替えは効かない——`trash` が無いと `filler` は 6 回中 5 回が空になる（**言いたいことの行き先が無いと、何も書かないほうへ倒れる**）。`_parse` は `full`／`action` のときだけ `filler` を発話に採り、`text` と `trash` は読み捨てる。ループ側は触っていない。

> v0.106（出-as 段 9b）で宛先の条件（出-ap）は外した。保留そのものが無くなったので、配る口で段を突き合わせる場面が無い。以下は当時の記録。

**話したいことは、それを言うのに誰が要るかで分かれる（出-ap・2026-09-23）。** 在席の注記は誰かを**呼ぶ**ことだけを止めており、**何を話してよいか**は誰も見ていなかった。

| 段 | 条件 | 機械が見るもの | 例 |
|---|---|---|---|
| 0 | 誰もいなくても | — | 独り言 |
| 1 | 誰かいたら | （配る口は在席が立ち上がった瞬間にしか呼ばれないので、ここでは常に満たされる） | タイマーが鳴っている |
| 2 | 家族がいたら | 在席に `person_id` を持つ人が居る | 家の記録・予定・メモ |
| 3 | 特定の誰かがいたら | その人が在席（器は `pending_speech.target_person_id`） | その人への申し送り |

段は**きっかけの札で一律に決める**（`core/audience.level_of`）——`[メモ]` は段 2、ほかは段 1。中身ごとに主LLM へ決めさせる案は、測る必要があるので見送った。`pending_speech.audience` に持ち、配るとき（`_release_pending_speech`）に**段 2 以上だけ**在席と突き合わせる。満たさないものは**消さずに残す**——次に家族が確認できたときに配る。

**段 1 をここで見ない**のは、`presence_status()` に載るのが顔の照合が付いた人だけだからである。見ると、誰か分からない相手のときにタイマーの知らせまで止まる。名前の分からない在席者（出-ae-は の札）は `person_id` を持たないので、段 2 は満たさない。

## 直近のやりとりは、人との言葉だけ

**やりとりの枠に人以外が混ざっていた（出-ar・2026-09-24）。** 起点を数えると **発話 309・情動 175・機器 130** で、人以外が半分を占める。実機 15:49 の `[直近のやりとり]` は `[タイマー]` の 1 行だけで、**人とのやりとりが 1 行も無いのに枠の名前は「直近のやりとり」**だった。

機器（タイマー・メモ・入室）は**起きたこと**、情動は**湧いたこと**で、どちらもやりとりではない。`[直近のやりとり（人との言葉だけ）]` と `[そのあいだに起きたこと]` の 2 枠に分ける。順は、人との言葉が先。どちらかが空なら、その枠ごと出さない。id はどちらの行にも残す（判定と申告が名指せる）。保留（話したかったが話せなかったこと）と「届いたもの」は、もともと別の枠にあるので触らない。

もとは「相手：」ではなく「きっかけ：」と書き分けていた（実機 2026-09-13 21:21：自分の内側や機器の出来事が誰かの発言に読めた）。枠ごと分けたので、守りたいことは同じままより強い形になった。

**これは読みやすさの直しである。** 枠を分けても、つなぎのあとの挨拶のやり直し（出-aj #4）は止まらない——**言葉の側は 8 通り測って全部 6/6**（W の `すでに伝えた一言`／可変部の行／`[返事]` の指示／その両方／`[人の言葉]` にやりとりの形で添える／静的核の `voice` に書く／W を時間順に 1 本化／この枠分け）。置き場所でも形でも統合でも止まらない。**残るのは機械側**である。

## 更新履歴

> v0.106：出-as（話していいかの決まり）を反映した（2026-09-26・`18bbec5`〜`94435da`）。「話す／話さない」の図と表を作り直し（窓・情動の軸・独白・知らせは記録だけ）、保留・宛先の条件・静穏時間の門・知-h の窓を外したことを、当時の節の頭に注記した。
> v0.105：出-ag-ろ——確認待ちのあいだに「掛けた」と言わせない 4 つ（09-21 の入口の守り・機械が読む問いと、09-25 の返り文・書き換え後の照らし）を書いた。枠を反復ごとに写す直しと検査に確認待ちの行を渡す直しは打ち消した（`根拠台帳` §49）。「整合チェック」を「発話前の検査」へ改名（2026-09-25）。
> v0.104：出-aq——つなぎの声を待たない・つなぎを `say` と別の道具にし主LLM の会話に置く・6 つの担い手に口・重複は機械が落とす（2026-09-25・`30a12fa`・`00b4430`・`ce6d612`）。あわせて「`effort=low` は速い」前提が実機と合っていない事実を記した。
> v0.103：出-ar——直近のやりとりを人との言葉だけにし、機器と情動は `[そのあいだに起きたこと]` へ（2026-09-24・`983e650`）。
> v0.102：出-aj（つなぎを `filler` へ・`trash` を置く）と 出-ap（宛先の条件で保留を絞る）（2026-09-23・`266e1d7`・`b774dbd`）。
> v0.101：出-ak——相手で口調を決める（誰へ言ったか・大人の印・`ME.md` の行を返事へ）（2026-09-23・`876843b`）。
> v0.100：出-ai——引き方の 1 行が採点ぶんと関連ぶんを分けて数える（2026-09-22・`26c9887`）。
> v0.99：出-am——身元を否定されたら在席から外す。失効が名前の無い在席者も畳む（2026-09-22・`c210c68`）。
> v0.98：出-an——主LLM の写真の見立ても `say` と同時に申告させ、在席へ流す（2026-09-22・`7419dbd`）。
> v0.97：出-ae-は——写真の見立てを在席へ流し、名前の分からない在席者を持てるようにする（2026-09-22・`0a113ce`）。
> v0.96：出-al——能力をいまの道具で数え、使えない道具を 1 行で伝える（2026-09-22・`f4f57ab`）。
> v0.95：出-ah-ろ——思い出すことを身体として名乗り、`[返事]` の行を引き直しの後にする（2026-09-22・`463139e`）。
> v0.94：記-k——想起に語の軸を足し、軸ごとの順位で底上げしてから採点する（2026-09-22・`a497fcf`）。
> v0.93：出-ah——思い出し方を変える 4 つの道具（`recall_as`／`recall_deeper`／`recall_when`／`recall_recent`）と、引き方の 1 行・規則を足した（2026-09-21・`23390f5`）。
> v0.92：W の過去の記憶を全文に（120 字の切りをやめる・2026-09-20・`0e4bae0`）。
> v0.91：出-ae(2)（直近の窓に時間の上限 5 分・`ba2bbd4`）（2026-09-19）。
> v0.90：出-ag（帰りの先導文に基準文）・出-ae(1)（在席の注記に「名前で呼ばない」）（2026-09-19）。
> v0.89：知-w：調停が名乗りを読み、在席があるときだけ話者に付ける（2026-09-19）。
> v0.88：出-z：上限到達の指示に「した」と言える基準（2026-09-18）。
> v0.87：出-ab：タイマーがあるときの操作の言葉は不在の門でも通す（2026-09-18）。
> v0.86：情-n（帰りの反復では沈黙の依頼を読まない）・出-aa（タイマー中の操作の言葉の light は full へ）・情-l-ろ（明示の沈黙が退室で解けたら記録も消す）（2026-09-18）。
> v0.85：出-x-ろ：`needs_tools` の守りを道具の帰りの反復では掛けない（2026-09-18）。
> v0.84：出-x：タイマー・アラームの帰りは想起なし・返りを受ける先導文・返った道具を候補から外す（3 種の W で 24/24）。出-y：確認を機械の状態に（`confirm_state`・`confirm`／`decline`・`confirmed` 撤去）（2026-09-18）。
> v0.83：「いま道具から返った」を W の最上部に（出-x・実験で決定・2026-09-18）。
> v0.82：情-l：沈黙が解ける条件を居るかの層で（タイマー由来は在席で解けない）（2026-09-18）。
> v0.81：出-w：情動の light 空は「黙る」で閉じる（2026-09-18）。
> v0.80：情-k（沈黙が明けたは黙っていた分だけ）・知-r（退室の知らせは独白）（2026-09-18）。
> v0.79：To-Be の図と表に話者の指定の寿命（知-t・2026-09-18）。
