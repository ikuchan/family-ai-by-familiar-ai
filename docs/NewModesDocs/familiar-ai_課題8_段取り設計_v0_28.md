# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.28）
...
| #11 | DIF／I 内部再設計（純イベント駆動3キューへ差し替え・#6 反復抑止を同梱） | **段階1・2 完了／段階3〜5 未** |
...
| 段階3 | AIF／DIF／完了 の3キュー結線（drive 発火・動体・deferred を各キューへ移設） | 未 |
| 段階4 | 二段生成（軽量つなぎ即答＋フル本応答） | 未 |
| 段階5 | 旧 `run()`・GUI アイドル分岐の撤去（Phase 6） | 未 |
