...
io/       出入口（`設計図` ③-2 の4つの口）
  ...
  aif.py            自律機構接続（AIF・T 接続）
  dif.py            機器接続（DIF・身体／MCP-音楽／MCP-検索／主LLM）
  oif.py            記憶接続（OIF・追記／読み出し／supersede／ベクトル埋め込み）
...
| 段 | 内容 | 出る先 | 挙動 |
|---|---|---|---|
| **環-e-い** | **OIF を作る**。記憶の追記・読み出し・supersede と**ベクトル埋め込み**を1つの口へ集める | `store/` の上に口を1枚 | 不変（同期のまま） |
| **環-e-ろ** | **DIF を作る**。身体（カメラ・マイク・スピーカー）・MCP-音楽・MCP-検索・主LLM への呼び出しを1つの口へ集める | `io/dif.py` | 不変（中は同期の転送） |
| **環-e-は** | **実体を切り出す**。生成器（GEN）・動作器（ACT）・想起（REC）を `loop/event_loop.py` から出す | `loop/generator.py`・`loop/actor.py`・`loop/recall.py` | 不変 |

**OIF を先にするのは、依存が最も少ないためである。** 記憶は呼ばれる側であって、他のコンポーネントを呼ばない。実測でも O 書込は外を呼ぶのが1箇所しかない。DIF はその次で、外部への呼び出しを1つの口へ寄せるだけなら中身の順序は変わらない。実体の切り出しは、口が揃ってからのほうが移す先が決まっている。
