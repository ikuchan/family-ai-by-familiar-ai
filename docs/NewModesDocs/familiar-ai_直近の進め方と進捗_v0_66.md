...
**次は 記-a-ろ（蒸留）の実装である。** **内省はツールを持ったエージェントとして専用ループで動かすと決めた**（`課題8 §7`）。 設計は決まっており（`課題8 §7` の「蒸留の設計」）、起動と骨格（記-a-い）は完了して実機でも確かめた。**Config 自己調整の器も作った**（3段の優先順位・範囲の登録制・接続情報は拒否）。内省が値を提案する部分は記-a-に で足す。着手の前に `課題8 §7` の「これからやること」を読むこと（溜まっている宿題がそこに並んでいる）。
...
- **感情の振れ幅・強度の tuning**（情-a の残り）。ループは閉じて実効があることを確認済みで、値の調整は「そのうち」。
...
- **話していないのに書き起こされた**（2026-07-30・実機）。「ご視聴ありがとうございました」「ありがとうございました」が入力として積まれ、ターンが走っていた。Whisper が無音や物音に字幕の常套句を当てる幻聴で、マイクが実際に拾った音ではない。silero-vad は音の有無しか見ないので、物音が区間として切り出されると書き起こしへ回る。実機15件に人がラベルを付けて測ると、幻聴は全件 `no_speech_prob` 0.722 以上、本物は全件 0.709 以下で分かれた。境目の直下 0.72 を `STT_NO_SPEECH_MAX` の既定として、これを超えた書き起こしを捨てる。`avg_logprob` は重なるので使えない（幻聴のほうが尤度が高いことすらある）。**境目の隙間は 0.013 しかない**ので、実機で取りこぼしか誤削除が出たら値を動かす。根拠は計測台帳 §20。

- **周囲の会話を拾う**：別の人の会話が書き起こされてターンを起こす。実機で、本人の「パジュ黙って」が届かず別の人の会話が入力になっていた。話者の分離は 知-d（登録）待ち。
...
- **1つの発言に2回答えた**（2026-07-30・GUI）。**真因を特定して塞いだ**（2026-07-30）。確定した書き起こしを二人が積んでいた。`realtime_stt_session._committed_relay` は確定文を表示用の差し込み口へ渡した直後に入力キューへも積むが、`gui.py` はその `committed_queue` として `_input_queue` そのものを渡しており、差し込み口に配線された `_on_realtime_stt_committed` もキューへ積んでいた。よって音声入力1件につき必ず2件が積まれ、ターンが2回走る。当時入れた GUI の重複除去が効かなかったのは、セッション側の投入がその窓を通らないためである。表示の口からキュー投入を外し、入力をセッションの1本にした。あわせて GUI 側の重複除去（`DuplicateInputFilter`・`INPUT_DEDUPE_WINDOW_SEC`）を撤去した。**重複除去は音声側の課題で**、書き起こしどうしの重複はセッション内の 3 秒の窓と `VoiceLoopGuard` が済ませている。キーボードは人が意図して打つので弾かない。経路は『音声入力からGUIへの経路』v0.3 に記録した。
...
- **`getattr` 形は `grep` に出ない**。`getattr(self, "_x", None)` は `self._x` の検索に引っかからない。環-c でこれを見落とし、毎ターン使われている `_concerns` を「使用0件」と誤判定して消した。`if ... is not None:` で守られているため、消しても例外は出ずテストも通る。**`ruff`・`mypy`・テストのいずれも捕まえられない。** 使用0件の判定には `getattr` 形を必ず含める。

- **配信ゲートが在/不在の層を見ていない**（2026-07-28、環-b の実機確認で判明）。`agent.py` の `_social_presence_permission()` が在席の証拠とするのは、PMM が返す人物 id と、直近5分以内に人が話しかけてきたことの二つだけで、`PresenceSensor`（YOLO）を読まない。知-b で `Tonic.scan_presence()` は二層（在/不在は YOLO、誰かは PMM）へ直したが、配信ゲートはその外に残っている。
  ...
  **配る起点そのものは動く。** 在席がゼロから立ち上がった 162 ミリ秒後に配られており、環-b が求める起点は実機で成立している。届かないのは配った後の発話である。ただし配った分は元の観測ごと `pending_speech` から消えるため、中身が入れ替わりながら保留が積み直される。

  一言話しかければ直近5分の逃げ道が効いて話せるようになる。実機確認がたいてい発話から始まることが、この欠陥がこれまで表に出なかった理由だと考えられる（未確認）。

  **修正は 知-d（顔と声の登録）を実装してから行うと決めた**（2026-07-28）。それまで 環-b は「起点は動くが、配ったものが人に届かない」状態で止まる。
...
> v0.58：**内省をツール駆動にする決定**と、**調停の時間切れを 5.0 秒にして Config へ出したこと**を反映。申し送りにあった「調停の時間切れの値の決定待ち」は解消した。
> v0.57：**記-a-ろ（蒸留）の設計決定と、Config 自己調整の器の実装**を反映。既存の日次要約が「終了時に作る」形で、日次の内省として回っていたわけではなかったことも記した。
> v0.56：**課題の番号の振り直し**を反映し、現在地の節を書き直した（古くなっていた「直近の5項目」と13項目の着手順を落とした）。
> v0.55：**実機確認の結果を反映**。#12a のあとも毎ターンの処理が壊れていないこと（背景タスクの失敗0件・6ターン）、入退室が QD へ積まれること、REST 内省の起動が通ることを確かめた。申し送りに `FAMILIAR_ENV_FILE` の上書き順を追加。
