# familiar-ai イベント駆動ループ（#11・段階5）（v0.77）
...
| 声がしたが応じられない | 起点が発話の返事が「聞く相手が居ない」で保留になったら SEEKING を発火閾値の半分〔仮〕押し上げる（案ア）。発火は tick が決め、情動の求めで調停が `look` を選ぶ |
