...
**出-a の TTS をローカル化した**（2026-07-30・記-a の途中で前へ動かした）。Style-Bert-VITS2（声＝`jvnv-M2-jp`）を別プロセスの HTTP サーバーとして動かす。実測は読み込み 2.8 秒・合成 0.11〜0.42 秒で、実機で音が鳴り声が男性に変わることを確認した。**ElevenLabs の無料枠に縛られなくなった。** 残るは STT である（`計測・設定値 根拠台帳` §9・`課題8 §7`）。
...
- **STT の利用枠**：ElevenLabs の無料枠を使い切った（10,000/10,000・**2026-07-29 10:47 に回復**）。TTS と STT が同じ枠を使う。**音声を使う実機確認は、それまでできない**。設計（`設計図_Mermaid` [D-知覚]）は音声 I/O のローカル化を確定させている（STT＝faster-whisper／TTS＝Style-Bert-VITS2・jvnv-M2-jp）が、着手は順10（出-a）。
...
- **STT の利用枠**：TTS をローカル化したので**発話では枠を使わなくなった**。ただし STT（`stt.py`・`realtime_stt.py`）は ElevenLabs のままなので、**聞き取りには枠が要る**。
