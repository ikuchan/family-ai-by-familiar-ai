...
次の一歩は**棚卸し**である。順序方針をリファインメントし（段取り v0.24）、大きな挙動変化（Drive 発火＝起動源・dynamics 接続・拡散想起・データモデルの深い作り替え）は後回し、小さく正しく先行、リファクタリングは 境界R→D→内部R に割り、loop に触る作業はまとめて後回し、顔登録・識別・設定値入力は最後、と決めた。これに沿って、(a) 小さくて正しく直しておきたい機能・既知の危うい所、(b) loop に触らずに引ける境界R の単位（store/io/core のつなぎ目）を洗い出し、近い所のバックログを具体化する。ここまでで、5軸想起（r/t/e/a/p）・書き込み PAD 化・A＝novelty 配線・在席者相関 p（slice-1/slice-2）は実装済み。境界の一部（`store/` 分割・`loop/evaluator`・`legacy/`）と min_score の合成床化も済んでいる。以下の実機確認は通常の利用の中で溜めておく。挙動変化を伴う P-1（活性の想起接続）・W2b-2（評価器の PAD 出力）・スライス3（e 軸の接続）は、気分一致想起が効いているか、e が拒否権になっていないか、半減期短縮で古い記憶が沈みすぎていないかを対で見る。
...
- P-4（5軸合成）は完了。r・t・a・e はスライス3 で、p は slice-1（score）＋slice-2（候補集合拡張）で揃った。
- p 軸（在席者相関）はスコアラに実装済み。実挙動での発火は在席入力（顔登録＝S・知覚）が埋まってからで、S は R と D の後。situated V2 の slice-3 以降（視点列からの関係生成・`person_id` 削除・旧 `_remember` 撤去）は同じく知覚が視点列を埋めることに依存する。
