# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.20）
...
2. **`memory.py` の分解**。アクセス層の完成（`by_vector`／`by_date` 等の専用メソッド・構造化値で受ける）と、時刻の一元化（どの時計を使うかを一箇所に）。
3. **`agent.py` の副作用境界の切り出し**。永続化と発話。
