...
- **1つの発言に2回答えた**（2026-07-30・GUI）。**真因を特定して塞いだ**（2026-07-30）。確定した書き起こしを二人が積んでいた。`realtime_stt_session._committed_relay` は確定文を表示用の差し込み口へ渡した直後に入力キューへも積むが、`gui.py` はその `committed_queue` として `_input_queue` そのものを渡しており、差し込み口に配線された `_on_realtime_stt_committed` もキューへ積んでいた。よって音声入力1件につき必ず2件が積まれ、ターンが2回走る。当時入れた GUI の重複除去が効かなかったのは、セッション側の投入がその窓を通らないためである。表示の口からキュー投入を外し、入力をセッションの1本にした。あわせて GUI 側の重複除去（`DuplicateInputFilter`・`INPUT_DEDUPE_WINDOW_SEC`）を撤去した。**重複除去は音声側の課題で**、書き起こしどうしの重複はセッション内の 3 秒の窓と `VoiceLoopGuard` が済ませている。キーボードは人が意図して打つので弾かない。経路は『音声入力からGUIへの経路』v0.2 に記録した。
