# familiar-ai 音声入力から GUI への経路 v0.8
...
パパだよ」が「パパだよ」だけになった）。v0.7 で 3 つ入れ、v0.8 で 1 つを撤去した。(1) `create_realtime_stt_session` が
「名前：」の綴りを**全部**空白区切りで `STTConfig.hotwords` に、並びを `STTConfig.names` に入れる。
(2) 書き起こしに上がった 2 語目以降の綴り（はじゅ・パチュ・パジュー）は `stt_rules.normalize_name` が
先頭の綴りへ直す。完全一致だけ当て、表に無い語（体重）は触らない。直したときは INFO
（`STT: 名前を「パジュ」に直した：パジュー`）。沈黙の守り（`core/silence_rules.names_me`）の
ゆるい読みはそのまま（`イベント駆動ループ` v0.74）。

**書き起こしの道具へ文を渡さない**（v0.8・2026-09-20）。v0.7 は名前入りの短い例文を faster-whisper の
`initial_prompt` へ渡したが、**渡した文がそのまま書き起こされて人の発話になった**。実機 17:22〜17:26 に
「パジュ、3 分測って。」（例文の後半・11 字ちょうど）が繰り返し会話入力として積まれ、GUI に並び、
タイマーまで掛かった。10.7 秒の音や 30 秒の窓がこの 11 字になっており、`no_speech_prob` は 0.02〜0.22 で
「音声でないものを捨てる」門（0.72）は通り抜ける。`initial_prompt` は「直前の文脈」として働くので、
はっきりしない音に当たるとその文を書き出す。**文を渡す口は、いまどちらの経路にも無い**（常時集音の
`local_stt._transcribe` と、録音ボタン式の `tools/stt.py`）。渡すのは `hotwords` の語の並びだけで、
語の並びは文ではないので反響しない。あわせて、**渡した手がかりが丸ごとそのまま返ってきたら捨てる**
（`stt_rules.is_echo_of_hint`・前後の空白と句読点は無視・綴り 1 語は人が名前を呼んだ場合があるので捨てない）。
名前の落ちやすさは v0.6 の状態へ戻る（知-z は開いたまま・`hotwords` と綴りの直しで様子を見る）。
...
| （`ME.md` の「名前：」） | — | 綴りの全部が `STTConfig.hotwords`（空白区切り）と `names` に入り、先頭の綴りが書き起こしの直しの正本になる。env からは与えない |
