# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.78）
...
| **出-c** | **モデル資源の共通化**（重みを持つ5つが1つの型枠へ） | **完了**（2026-09-04）。`Encoder` は実物を見て作らないと決めた。詳細は `モジュール分割設計` v0.13 |
| — | 小さな片付け（下表） | 完了 |
...
| **出-d** | **モデルへの問い方を揃える**（形のある答えを求める口・`backend.py` の分割と型付け） | 依存なし。**実機を要さない**。**AI 再選定の前に置く**。詳細は `モジュール分割設計` v0.14 |
