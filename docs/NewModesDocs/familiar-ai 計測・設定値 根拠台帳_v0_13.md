...
$mult(t)$ は蓄積式 $drive_i \leftarrow \mathrm{clip}(drive_i + rate \cdot mult(t) \cdot learn_i \cdot g_{D,i}(M) \cdot P_T)$ に入る係数で、器はあったが値が 1.0 のまま動いていなかった。旧 `DesireSystem`（15欲求）には静穏時間の倍率 0.45 が実装されていたが、新 Drive5 とは排他なので既定 on の系統では効かない。
