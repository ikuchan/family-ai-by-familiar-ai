# familiar-ai 設計詳細：活性・O書込・知覚在席（定数台帳・現状コード所在・移行）（v0.19）
...
|---|---|
| センサあり | `room_occupied()`（滞留窓 120 秒内に人）**or** 人の声から `PRESENCE_VOICE_SEC`（60 秒〔仮〕）以内。**在席表は見ない** |
| センサ無し | 在席表に誰か **or** 声から 60 秒以内（従来どおり） |

声の窓はセンサの視野の外から話しかけられたときの補いで、5 分 → 1 分（`AgentConfig.presence_voice_sec`）。
