# familiar-ai ユースケース⑥：記憶を問う対話（想起の重みプロファイル切り替え）（v0.1）
...
- 想起＝[D-想起合成] `recall_score = r^(w_r) × M`（M＝(w_t·t+w_e·e+w_a·a+w_p·p)/(w_t+w_e+w_a+w_p)＝加重平均・ハイブリッド＝関連だけ乗算ゲート）。**重みプロファイル (w_r,w_t,w_e,w_a,w_p) は seed が運ぶ**（既定＝**基底プロファイル** (1,1,1,1.5,1.0)→在席者がいる間は r·(t+e+1.5a+p)/4.5、在席者ゼロは p を外し (1,1,1,1.5)・現コードとは非一致。w_p は在席者相関＝[D-在席相関]）。
...
- **〔未対応・課題6/8〕実装**：評価器が発話から重みプロファイルを出す（プロンプト／構造化スキーマ）。現行 recall（`_compute_final_score`＝積・固定重み）を**ハイブリッド合成（関連ゲート×加重平均）＋重みプロファイル可変**へ拡張。