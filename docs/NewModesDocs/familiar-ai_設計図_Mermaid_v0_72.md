# familiar-ai 設計図（Mermaid一式・v0.72）
...
graph TD
  ...
  MLLM <-.->|"指示と応答：prompt and reply"| DIF
...
**DIF を通らずに外部へ出る線は置かない。** 生成器も動作器も、外部へは DIF 経由でしか届かない。返るものはすべて DIF から IIF の待ち行列へ入る。
