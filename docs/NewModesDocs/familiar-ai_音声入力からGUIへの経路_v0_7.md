# familiar-ai 音声入力から GUI への経路 v0.7
...
**名前を手がかりとして渡し、聞き違いの綴りは直す**（v0.6・2026-09-17／v0.7・知-z・2026-09-19）。
「パジュ」は日本語の語彙に無く、実機で 体重／パチュー／はじゅ と書き起こされ、「パジュ、静かにして」
の沈黙依頼が名前の守りで 3 回とも落ちた（09-17 15:31〜15:32）。v0.6 は `ME.md` の「名前：」の
先頭 1 語だけを `hotwords` に渡したが、large-v3 は名前を落としもする（09-19 に 2 回、「パジュ、
パパだよ」が「パパだよ」だけになった）。v0.7 で 3 つにした。(1) `create_realtime_stt_session` が
「名前：」の綴りを**全部**空白区切りで `STTConfig.hotwords` に、並びを `STTConfig.names` に入れる。
(2) `_transcribe` が先頭の綴りから組んだ短い例文（`stt_rules.initial_prompt_for`・「パジュ、
こんにちは。パジュ、3 分測って。」）を faster-whisper の `initial_prompt` に渡す（長い例文は無音で
反響しやすいので 2 文にとどめる）。(3) 書き起こしに上がった 2 語目以降の綴り（はじゅ・パチュ・
パジュー）は `stt_rules.normalize_name` が先頭の綴りへ直す。完全一致だけ当て、表に無い語（体重）は
触らない。直したときは INFO（`STT: 名前を「パジュ」に直した：パジュー`）。沈黙の守り
（`core/silence_rules.names_me`）のゆるい読みはそのまま（`イベント駆動ループ` v0.74）。
効果（書き起こし 20 回中の残存率・例文の反響の有無）は実機で測る。
...
| （`ME.md` の「名前：」） | — | 綴りの全部が `STTConfig.hotwords`（空白区切り）と `names` に入り、先頭の綴りが `initial_prompt` の例文と書き起こしの直しの正本になる。env からは与えない |
