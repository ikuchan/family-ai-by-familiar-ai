...
旧 R/S/V/E は **[D-I内部]** で再編。**I は純イベント駆動**（時計を見ない）。入口は **3つのキュー**——**AIFキュー（T からの情動）／DIFキュー（機器イベント）／完了キュー（LLM・MCP の非同期結果）**（**③-2 の1本へ統合済み**・環-f-い-1・2026-09-11）。これらに書き込む **3機構（AIF・DIF・資源ハンドラ＝同色）**が同期非同期を変換し、**ループ核（LPM）が3キューを drain して回す**（調停・想起を内包、生成器・評価器・動作器・統合保守器を切り出す）。**T レジスタには触れない**（I が受けるのは情動・[D-B分離]）。LLM・MCP は内部資源。
...
**3つのキュー書込機構（AIF・DIF・資源ハンドラ）は同色**＝同期非同期の変換口。ループ核は **3キューの union を待ち、来たどれでも起きる**（時計は見ない。**いまは1本の待ち行列を待つ**・環-f-い-1）。生成器／動作器が投げた LLM・MCP は**完了キューで戻り、O に書く**（完了は関連＋未解決で O の open 意図を想起で再会・相関ID なし・[D-単一想起]）。**情動は中身を持たないので取り込み時に想起（O→W）で状況づける**。**タイマーは T が due で情動を発火**（I は時計を持たない）。外部接続は **DIF（機器）だけ**、I 外接続は **T（情動 via AIF）のみ**（T レジスタには繋がらない）。資源ハンドラ＝LLM担当・実行担当の総称。
...
**【実装済み・環-f-い-1／2026-09-11】** 待ち行列は1本（`loop/event_loop.py` の `_triggers`）で、
運ぶ器は `Trigger`（きっかけ）。順序づけは `_take_trigger()` の1箇所にある。調査中に届いた
情動・機器は**保留箱**（`_held`）で待たせる——3本だったころ「キューに残す」ことで待たせて
いたのと同じ意味である。**【環-f-い-2／2026-09-11】会話入力も列を通る**（`push_utterance`）。反復は駆動体の上で
回り、呼び手は `Future` でその反復の出力を待つ——`begin_request` が返していたのも求めの
終わりではなく最初の反復の出力だったので、**呼び手の約束は変わっていない**。

会話入力がキューを通らず同期の関数呼び出しで入る形（`agent.py` から `begin_request` を直接呼ぶ）も、これで消える。**マイクは身体の一部として DIF を通る。**
...
> v0.96：**③-2 の「1本の待ち行列」が実装済みになった**（2026-09-11・環-f-い-1）。3つのキュー
> を1本にし、器を `Trigger`（きっかけ）へ改名した。③ 側の「3つのキュー」の記述にも統合済みの
> 印を添えた。**会話入力だけがまだ同期で横から入る**（環-f-い-2）。

> v0.95：**Drive の dynamics が繋がっていることを台帳へ書いた**（2026-09-10・群D の点検）。
> `accumulate`／`fired`／`discharge` を `tonic.py` が回し、`accumulate` は `g_d(mood, cfg)` を
> 掛けている。台帳の前のほうにある「蓄積と mood 変調は未実装」は B-2 当時の記述で、いまは
> 当たらない。`感情ループ全体像`（v0.8）の表も同じ理由で直した。

> v0.94：**撤去台帳の在り処と名前を直した**（2026-09-10・環-e-に の に-5-に-2／後始末）。W を組み立てるのは
...
> 誤りだった。あわせて、器だけが残っていた `familiar_agent/workspace.py` を
> **`coalition.py`** へ改名した（`workspace.py` が2つある状態を解消）。

> v0.91：台帳の**直近のやりとり**を、判定者の差し戻しに合わせて直した（`根拠台帳` §29）。続き先の判定は軽量LLM の専用の仕事になり、`say` の `follows` 欄は撤去した。

> v0.90：「store と I/F」台帳へ**共起の移行**を追記した（060・段 5）。`設計方針_MI間の関係` の5 段がすべて実装済みになり、記録どうしの関係は `relations` と `relation_members` に集まった。

> v0.89：「store と I/F」台帳へ**直近のやりとり**を追記した（段 4）。`follows` で W から続き先を名指させ、継起を根までさかのぼって可変部へ逐語で載せる。つなぎを記録へ戻し、想起に出さない役割を二つにした。

> v0.88：「store と I/F」台帳へ**やりとりと継起**を追記した（段 3）。ターンの記録を順序つきの一つの関係として残し、答えの逐語を畳むのをやめた。起点を控える並びに載せたので、問いが拡散想起の母集合にも入るようになった。

> v0.87：「store と I/F」台帳へ **MI 間の関係**を追記した（058・059）。記録どうしのつながりを二項の列から多項の関係へ移し、`superseded_by` 列を落とした。役割 `旧` が「もう現行ではない」を表し、種類は理由を言うだけである。`[D-発話]` の「応答で根づきを落とす（supersede）」を、答えた問いも残る形へ書き改めた。想起の共通フィルタと「畳んでも面は残る」の根拠も、列でなく関係を指すよう直した。
