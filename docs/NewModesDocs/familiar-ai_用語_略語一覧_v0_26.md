...
| MI／記憶モデル | 基底プロファイル | Base profile | — | 重みプロファイルの既定値＝(w_r,w_t,w_e,w_a,w_p)=**(1,1,1,1.5,1.0)**＝在席者がいる間は base＝r·(t+e+1.5a+1.0p)/4.5、在席者ゼロは p を外し r·(t+e+1.5a)/3.5（$w_e=1$・$w_a=1.5$・$w_p=1.0$ は加算部係数・$C=2$・**現コードとは非一致**）。指定が無い想起で使う。[D-想起合成]／[D-在席相関]。 |
...
| MI／記憶モデル | 感情一致スケール | sigma | σ | $e=\exp(-D/σ)$ の σ。小＝感情差にシビア／大＝近縁も仲間。**Config 調整可・範囲 0.3〜3.0**・既定 1.0（[D-想起合成]／[D-自己認識分離]）。 |
| MI／記憶モデル | 軸重み | axis weight | λ | 感情一致の距離 $D=\sqrt{Σλ_i Δ_i^2}$ の各 PAD 軸の重み。**Config 調整可・範囲 各 0.1〜3.0**（下限>0 で軸を完全には消せない）・既定 1.0（[D-想起合成]）。 |
| MI／記憶モデル | コサイン伸長下端／上端 | cosine stretch bounds | c_lo／c_hi | 関連 r の固定係数 min-max 伸長 $r=clip((cos-c_{lo})/(c_{hi}-c_{lo}),0,1)$ の下端・上端。生コサインは無関係でも正に偏るため他項と揃う分布へ引き直す。**Config 調整可**（c_lo:0〜0.7／c_hi:0.5〜1.0・c_lo<c_hi）・初期値は課題7（[D-想起合成]）。 |
...
| 想起 | 埋め込み平均中心化 | Mean centering | — | コサインを取る前に全埋め込みから共通成分（平均ベクトル）を引き L2 正規化する前処理。異方性（cone 効果）で生コサインが高位圧縮する問題への対処。平均ベクトルは固定保存・低頻度で再推定。r／在席者相関 p／声紋照合に共通適用（[D-想起合成]）。 |
