# familiar-ai 設計詳細：活性・O書込・知覚在席（定数台帳・現状コード所在・移行）（v0.22）
...
（`Tonic._expire_presence_table`）が在席表の全員を `PersonMemoryManager.mark_absent` で消す。**話者の指定
（`_speaker_id`）は残す**——顔と声の登録（知-d）までは `/speaker` が唯一の手がかりで、次に声が来たとき誰かを
知るため。`person_left`（両方消す）とは別の口。センサが無い構成では失効しない（在席表が唯一の情報源）。
