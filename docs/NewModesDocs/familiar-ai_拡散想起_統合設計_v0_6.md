...
拡散で W に足す想起 MI は、**既存 MI をそのターンの W（一時的な派生ビュー）へ引き込むだけ**で、そのストア行の `根づき`／`a0` を**書き換えない**。`a0=0` は**そのターンの W 内での重み**を指し、ストアへの書き込みではない。
...
3. **(A) 共起辺【済】**：`RelationStore.cooccurring`（共起≥2 の集まりの要素 MI・現 W と `self_model` 除外）。**種類 `共起` で絞る**（絞らないと同じターンの問いと答えが共起として返る）。060 以前は `diffuse_store.cooccurring_mi_ids` だった。
...
> v0.6：**撤去した略語 WR を名前から落とした**（2026-09-08・環-g 段い）。`agent._record_wr`
> は `_record_cooccurrence` へ、`combine_wr_ids` は `combine_cooccurring_ids` へ改名した。
> 中身（共起の記録）は 060 で種類 `共起` の関係へ移っており、変わっていない。（この項は当時 file 名だけを上げて履歴を落としていた。2026-09-10 の点検で気づき、git から拾って遡って書いた。）
...
> v0.5：**共起の器を関係へ移したことを反映**した（2026-09-07・060）。`wr_records` は
> 種類 `共起` の関係になり、`wr_store.py` は撤去した。共起の検索は `RelationStore.cooccurring`
> で、**種類で絞る**（絞らないと同じターンの問いと答えが共起として返る）。
