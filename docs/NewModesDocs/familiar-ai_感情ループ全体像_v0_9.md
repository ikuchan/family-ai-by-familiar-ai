...
- **`FIRE → EVAL → OBS`（感情の生成）**：評価器（軽量LLM）が現ターンの遣り取りを P/Pn/Dom で評価する。A（高ぶり）は機械値で、**a0 と同じく seed 種別で作る（内容系＝novelty＝1−近傍K件の類似度平均／カメラ起点＝見え驚き Ŝ）＝keyword ではない**。値踏みゲート A_gate=0.25 未満は評価器を呼ばず、**P/Pn/Dom を未測定のまま残す**（050・2026-09-03）。
  **気分 M で埋めない。** 埋めると「測ったのか埋めたのか」が後から見分けられず、感情軸の母集合が一点に潰れる
  （実データで 6433 行のうち 2941 行が PAD 全部 0.5＝ゼロベクトルになっていた）。未測定は
  `emotion_p`／`emotion_pn`／`emotion_dom` を NULL で保存し、`emotion_vec` も作らないので、
  感情軸の一次絞り（`emotion_vec IS NOT NULL`）から自然に外れる。**A は機械値なので常に入る。**
  埋め直すのは REST 内省の仕事である（記-a・層 1「出来事」を畳む仕事の一部）。評価器が失敗したときも同じく未測定にする。評価器は W を入力にとらず、気分をベースに現ターンから感情を出すので、記憶が無くても感情は生まれる。
...
> v0.9：REST 内省の仕事を 4 層（出来事・自己像・設定値・能力）の語で書いた（2026-09-14）。
...
> 見分けられず、感情軸の母集合が一点に潰れる（実データで 6,433 行のうち 2,941 行が PAD
> 全部 0.5＝ゼロベクトルになっていた）。（この項は当時 file 名だけを上げて履歴を落としていた。2026-09-10 の点検で気づき、git から拾って遡って書いた。）
