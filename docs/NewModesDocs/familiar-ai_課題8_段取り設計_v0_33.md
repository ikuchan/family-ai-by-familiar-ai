# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.33）
...
- **想起の候補集合が1軸だけだった**：設計 [D-想起合成] は**多軸 union 一次絞り**（重み>0 の各軸で `ORDER BY … LIMIT N` して UNION）を定めているが、実装は関連軸（`by_vector`）しか作っていなかった。新しさは候補に入ったあとの並べ替えにしか効かず、直前の会話が候補にすら入らない。`by_time`（基準時刻に近い順）を足して union（活性軸は使わないと決定・感情軸は未実装）。乗算ゲートは触っていない（直近なら $M\approx0.7$ で、関連が低め $r=0.3$ でも $score\approx0.21$ となり下限 0.05 を大きく超えるため、候補集合の欠落で説明が付く）。
