...
**相対の向き（2026-09-16・知-m ③）**：`look` は `pose`（定点名）のほか `direction`（右／左／上／下）を受け、いまの向き（`CameraTool.position()`）から**その方向で最も近い定点へ絶対移動**する（`poses.pose_toward`）。名前の無い向きへは行かない（設計は変わらない）。右＝pan 正〔仮・`RIGHT_IS_POSITIVE_PAN`〕。その方向に定点が無ければ「知っている場所」を挙げて動かない。
