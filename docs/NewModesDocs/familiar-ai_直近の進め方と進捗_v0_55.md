...
**実機で確かめたこと（2026-07-29 20:37〜20:46）**。#12a のあとも毎ターンの処理は壊れていない。MCP は 3/3 繋がり、人の発話に返事が返り（`結末=発話` が6回）、`search_deferred` → `fetch_deferred` → 発話の3反復が通り、入室をきっかけに `see` が動いた。**`Post-response pipeline failed` は0件**で、今日いちばん手を入れた `_run_post_response_pipeline` が6ターン回って一度も失敗していない。あわせて、申し送りにあった**入室と退室が QD へ積まれること**も両方とも通った（入室 → `see` → 発話、退室 → `recall` → 発話）。
