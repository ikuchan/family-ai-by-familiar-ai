# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.53）
...
| 知-e | D-rel（関係の加工） | 知-d |
| 環-e | I の口を作り実体を切り出す（OIF → AIF → DIF → 実体） | 依存なし。**挙動を変えない**。OIF は完了 |
...
| 環-e-ろ | **AIF を作る**（T ↔ I の情動発火と Nudge を1つの口へ） | ← いまここ |
| 環-e-は | **DIF を作る**（身体・MCP-音楽・MCP-検索・主LLM への呼び出しを1つの口へ。中は同期の転送） | 未着手 |
