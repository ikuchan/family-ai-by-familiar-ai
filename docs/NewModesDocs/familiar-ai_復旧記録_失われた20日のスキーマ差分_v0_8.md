...
| 042 `drop_observations_person_id` | **復元済み**（2026-09-01） | 記-d（`person_id`）。**記-d はこれで完了** |
...
**所有者絞りは、そもそも人を分けていなかった。** 8月3日のダンプで `observations` 5080 行の
うち 4904 行（96.5%）が既定値 `default` のままだった。
...
持たない観測を拾う」役目を残していたが、生存する観測 2672 件の**全件**が situated 行を持つ。
役目が消えているので、フォールバックからも所有者絞りを外した。課題8 の C-1 の申し送りは
この実測で取り消した（v0.60）。

**重複判定の30秒窓だけは絞りを保ち、`writer_id` へ移した。** 別の人が同じ言葉を30秒以内に
言ったものは重複ではない。過去データでは別書き手の衝突 0件・同書き手の重複 0件で、この
付け替えで見える挙動は変わらない（反証側も確認した）。

**本番の不具合が1件見つかった。** `recall_self_model` と `recall_curiosities` は
...
絞りが間違っていても緑になる**。列を落とす調査では、書き込み側が実際に何を入れているかを
実データで見る。
...
> v0.8：**042（`observations.person_id` の撤去）を復元し、記-d を完了した**（2026-09-01）。
> 所有者絞りは 96.5% が `default` に潰れて人を分けておらず、C-1 が残した「situated 行を
> 持たない観測を拾う」役目も母集合 0 行だった。フォールバックからも外し、重複判定の30秒窓
> だけ `writer_id` へ移した。撤去の途中で `recall_self_model` と `recall_curiosities` が
> 本番で常に空を返していたことが分かった。調査では、文字列連結で書かれた `INSERT` を
> 取り落として 15箇所と数え、畳み直して 27箇所に直した。
