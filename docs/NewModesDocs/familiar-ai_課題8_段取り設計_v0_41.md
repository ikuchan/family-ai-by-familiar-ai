...
### Phase 6：旧系統の撤去
- 内容：**#12a（完了）**＝旧経路そのものの撤去。旧 `run()`、アイドル自発系、`EVENT_LOOP`、呼ばれなくなった層（interoception／social_policy／appraisal／attention_schema／meta_monitor／default_mode／mental_state と `GlobalWorkspace` クラス）。**#12b（未着手）**＝廃止ストア（tape／memory_links／self_narrative／relationship_state→案A／pending_store→O open 意図）の移管完了と撤去。
- 依存：#12a は #14。#12b は移管先が要るので #2・#3・#15 の後。
...
**順7（#2 REST 内省）**である。順6 のうち撤去できる部分（#12a）は完了した。順3（#15）は実機確認で欠陥が出て #17 待ちになり、順4（#13）と順5（小さな片付け）は完了している。

**#12 は二つに割った**（2026-07-29）。§3 の Phase 6 は撤去対象に「廃止ストアの移管完了」を含むが、その中には `relationship_state`（#3 待ち）、`self_narrative`（#2 待ち）、`pending_store`（#15 待ち）のように、移管先がまだ無いものが混ざっていた。§7 の依存表は順6 の依存を「#14 のみ」としており、二つの記述が食い違っていた。割る境目は**移管を伴うかどうか**である。

| | 内容 | 依存 | 状態 |
|---|---|---|---|
| **#12a** | 旧経路そのものの撤去（旧 `run()`、アイドル自発系、`EVENT_LOOP`、呼ばれなくなった層） | #14 | **完了**（2026-07-29） |
| **#12b** | 廃止ストアの移管と撤去（`relationship_state`／`self_narrative`／`pending_store`／`memory_links`／`tape`） | #2・#3・#15 | 未着手 |

これは #8 を #8 と #17 に割ったときと同じ考え方で、依存しない半分を先に進めるためである。

**順3（#15）は起点だけが動いて止まっている。** 実機で、在席がゼロから立ち上がった 162 ミリ秒後に保留が配られることを確認した。ただし配った直後の発話が「聞く相手が居ない」で塞がれ、`pending_speech` へ積み直される。配信ゲート `_social_presence_permission()` が PMM（顔の照合）と直近5分の発話しか見ておらず、`PresenceSensor`（YOLO）を読まないためである。#8 で `Tonic.scan_presence` は二層へ直したが、この関数は取り残されていた。**修正は #17（顔と声の登録）の後に行うと決めた**（2026-07-28）。詳細は `直近の進め方と進捗` の申し送りにある。

そのうえで、**順番から外れて先に実装したもの**と、**溜まっている宿題**がある。着手の前にここを見ること。

| 溜まっているもの | 状態 |
|---|---|
| **#16 見回りの実機確認** | 実装は済んでいる（順9 を先に実装した）。**首が実際に回ることを一度も確かめていない** |
...
| **周囲の会話を拾う** | 別の人の会話が書き起こされてターンを起こす。**未着手**（話者の分離は #17 待ち） |
| **#13 の深夜の挙動** | 倍率は入れたが、夜間の発火間隔を実機で数えていない |

### 着手順（2026-07-28 決定）

前提として、旧 #8「在席と身元」を**二つに割った**。割る境目は**顔と声の登録に依存するかどうか**である。照合はギャラリー（登録済みの顔）との比較なので、登録が無ければ誰も同定できず、カメラが繋がっていても同じである。一方、設計が定める二層のうち**在/不在の層は identity を要しない**ので、登録が無くても作れて実機で確かめられる。旧 #8 は登録が済むまで丸ごと止まる課題に見えていたが、止まるのは半分だけだった。

| 順 | # | 項目 | 依存 |
|---|---|---|---|
| 1 | **#14** | **見る・首を振るをループへ繋ぐ**（`_ACTIONS` に `see`／`look`） | **完了** |
| 2 | **#8** | **在/不在の層**（登録に依存しない部分・**内訳は下記**） | **完了**（S2〜S4・実機確認済み） |
| 3 | **#15** | **保留していた発話を配る起点** | **#17**（起点は動くが、配ったものが人に届かない） |
| 4 | **#13** | 深夜の Drive 発火を抑える時間帯倍率（課題10 の `mult`） | **完了**（`mult_quiet`＝0.083・実機未確認） |
| 5 | — | **小さな片付け**（下表） | **完了** |
| 6 | #12a | 旧経路そのものの撤去 | **完了**（実機確認は段階2 の時点で1回） |
| **7** | #2 | REST 内省（D 加工） | **← いまここ**（順1＝起動と骨格は完了・実機確認済み） |
| 8 | #4 | D-mi（MI 畳み込み・蒸留） | **#2** |
| 9 | **#16** | 昼夜の見回り（ユースケース③） | **実装済み・実機未確認**（順番から外れて先に実装した） |
| 10 | #9 | 音声出力 | — |
| 11 | **#17** | **顔と声の登録**（旧 #8 の土台・登録に依存する部分） | 登録に応じられる人が揃うこと |
| 12 | #3 | D-rel（関係の加工） | **#17** |
| 13 | — | p 軸の実挙動・大人と子どもの口調・一定時間で忘れる在席 | **#3** |

順1 を先頭に置くのは、`see` と `look` が**旧 `run()` にしか繋がっていない**ためである。既定を反転した（#11 段階5）ときに新しい系統から目と首が落ちており、いまカメラを使う経路が無い。順6（撤去）を順5 の直後に置くのも同じ理由で、両方に目と首が揃った時点が旧経路を消せる最初の時点になる。

順7 の REST は「誰も居ないとき」に回す機構なので、在/不在を信じられる形で判定できなければ起動条件を書けない。順9 の見回りは、首を振ること（#14）と定点ごとに人が居るかを見ること（#8）の両方が要る。順8 と順12 は本体が前段待ちで、順序を入れ替えられない。

順10（音声出力）に依存は無い。ここに置いたのは順1〜9 が機構で順10 が仕上げという並びのためで、先に必要になれば前へ動かせる。順11 は登録に応じられる人が揃うという外部条件で決まるため、揃った時点で割り込む。

### 知覚のスライス（#14・#8・#16 の中身・2026-07-28 決定）

`知覚在席` §3-2 の二層のうち、**人物判定（誰か・InsightFace）を外した残りすべて**を、次の5スライスで進める。人物判定は #17（顔と声の登録）が持つ。

| スライス | 内容 | 課題 |
|---|---|---|
| S1 | 見る・首を振るをループへ繋ぐ。見た画像を VLM で言葉にする | #14・**完了** |
| S2 | 絶対 pan/tilt と定点 | #16 の土台・**完了** |
| S3 | 在/不在（YOLO・定点別 presence マップ・滞留窓）→ QD | #8・**完了（実機で通し確認済み）** |
| S4 | 見えの普通（DINOv2 の定点別 EMA）→ $\widehat{S}_{景色}$ | #8・**完了** |
| S5 | 見回り | #16・**実装済み・実機未確認** |
...
`ExplorationTracker`（相対移動の積算）は `look` から外したが、器の撤去は #12 で旧 `run()` のプロンプトごと落とす。
...
**在席の情報源を二層に分けた**：在/不在は `PresenceSensor`（YOLO・登録不要）、名前は PMM（顔の照合・`/speaker`）。照合が済んでいなければ「誰か」として、居ることだけ伝える。旧 #8 が挙げていた「退室が実機で起こせない」は、登録を待たずに動くようになった。
...
**#11 は段階5 まで完了している。** 残るのは旧経路の削除で、それは #12 が持つ。段階3 の動体検知は既定 off のままで、QD への移設も #12 に含まれる。
...
**アイドル自発系は3つの入口すべてにあった。** GUI だけが `EVENT_LOOP` のゲートで守られており、TUI と CUI はゲートが無く、T（Tonic）と旧 `DesireSystem` が二重に動いていた。#11 段階5 で既定を反転したとき、GUI にだけゲートを入れたためである。
...
| 順 | 内容 | 状態 |
|---|---|---|
| **1** | 起動条件と骨格 | **完了**（実機確認済み・2026-07-29） |
| 2 | 蒸留（その日の O から自己エピソードを作り、元を supersede） | 未着手 |
| 3 | open 棚卸し（孤児を Warn・機械判定のみ） | 未着手 |
| 4 | near-dup 統合・relation_key 語彙・Config 自己調整 | 未着手 |

**順1 を先に置いたのは、起動が正しく掛かることを先に確かめるためである。** この機構はこれまで一度も動いたことがなく、中身を作り込んでから起動の誤りが見つかると切り分けが難しい。

**在/不在は `PresenceSensor` で見る。** `_social_presence_permission()` は PMM（顔の照合）と直近5分の発話しか見ないので使わない（#15 の欠陥）。センサが無い構成では内省へ回さず従来どおり QA へ積む（「センサが無い」を「誰も居ない」と扱うと、カメラ無しで常に内省へ落ちる）。誰か居るときの REST は自発ターンのままにする（「休みたい」と人に伝えるのは自然な振る舞いである）。

### #14（見る・首を振るをループへ繋ぐ）

`tools/camera.py` の `see`（撮って見る）と `look`（ONVIF PTZ で首を振る）は実装済みだが、イベント駆動ループの動作の表 `_ACTIONS` に無く、**新しい系統から呼べない**。旧 `run()` には繋がっているので、既定を反転した時点（#11 段階5）で目と首が落ちたことになる。表に2行足して繋ぐ。
...
**`look` は相対のまま繋いだ。** 設計が定める絶対 pan/tilt と定点は、在席マップと norm と見回りが共有する構造なので #16 で扱う。

### #8（在/不在の層）の内訳

用語一覧が定める**二層**（在/不在＝T(G)・連続、誰か＝I・必要時）のうち、**在/不在の層**を作る。この層は identity を要しないので、顔と声の登録（#17）を待たずに実機で確かめられる。
...
在席がゼロから立ち上がった瞬間だけ配る。いまその瞬間を作れるのは `/speaker` だけで、在席が正しく上下しないと配れない。#8 で在/不在が動けば、実機で確かめられる。

### #16（昼夜の見回り）

ユースケース③ が定める、定点を巡って様子を見る動き。首を振ること（#14）と、定点ごとに人が居るかを見ること（#8）の両方が要る。
...
- **`unconfirmed` のときに丁寧語にならない**：`ME.md` は「相手が分からないときは大人として扱い丁寧に話す」と定めるが、話者が `unconfirmed` のときは効かず、入力の文体を真似る。名前が入っているときは効く。原因は未特定。この症状自体は #8 で扱えるが、直ったことの確認には相手を変えられることが要る。
...
- **一定時間で忘れる在席**：MI の想起に駆動される在席（活性の派生ビュー）は、記憶が人に紐づくこと（D-rel＝#3）に依存する。#3 の後に、どちらの層が寿命を持つかを決める。
