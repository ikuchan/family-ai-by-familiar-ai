# familiar-ai 設計詳細：活性・O書込・知覚在席（定数台帳・現状コード所在・移行）（v0.30）
...
| 意味づけ（何が変わったか） | **I 側（必要時）** | **即席＝YOLO（COCO 80 種・`PersonDetector.labels`）／背景＝VLM（scene.py extract_entities・utility backend）** | `see` の帰りは即席のラベルで見た印を書き、VLM は背景で返って印を差し替える（`イベント駆動ループ` v0.43）。**主LLM は写真そのものを受け取る**（その求めのあいだ・最新1枚）。ラベルは記憶の文（想起・埋め込み）の材料 |
...
### 3-2b. 「誰かがいる」と「知っている人がいる」（知-h・2026-09-13）

二層は判断の**順**にも効く。配信ゲート（`agent._social_presence_permission`・「聞く相手が居ない」）は **居るか → 誰か** の順で見る：(1) 在/不在の層が人を見ている（`PresenceSensor.room_occupied()`・YOLO・登録不要）、(2) 顔が照合されている（PMM＝知っている人）、(3) 直近 5 分に話しかけられた。どれかが立てば在席。以前は (2)(3) だけで、顔が未登録なら目の前に人が居ても「誰も居ない」になり、独り言が独白へ落ちた。主LLM へ渡す在席（`generator._present_ctx`）も、顔でも自己申告でも分からないが YOLO が人を見ているときは「誰か居るが顔は確認できていない」を返す。入退室・REST の内省（既に YOLO）、沈黙依頼の宛先（名前）、話者ゲート（知-f・身元の層）は変えない。

**居るかの正本は在/不在の層、在席表は「誰か」だけ**（知-p・v0.19・2026-09-17）。上の 3 つは OR だったので、
`/speaker パパ` で在席表（PMM）に入ったパパに**出る口が無く**（`PresenceWatcher` は未起動・`note_person_left` は
呼ばれない）、カメラが 2 分「誰も居ない」を見ていても自発が出た（実機 15:44）。改めた：

| 構成 | 居るか（`_social_presence_permission`） |
|---|---|
| センサあり | `room_occupied()`（滞留窓 **60 秒**内に人・v0.29 知-x で 180 → 60：1 フレームの誤検出が窓をまるごと延ばし無人でも 3 分「居る」になった。反応の鈍さを嫌い、静止している人の見落とし側の余裕を削った。動体イベントを在席の証拠に足す案（D）は「見落とし」側の手で、今回の誤検出には効かないので入れない）**or** 自分が話してから `PRESENCE_SAID_SEC`（60 秒〔仮〕）以内 **or** `/speaker` を打ってから 60 秒以内。**マイクで拾った声と在席表は見ない**（v0.21） |
| センサ無し | 在席表に誰か **or** 自分が話してから 60 秒以内 **or** `/speaker` から 60 秒以内 |

**マイクは在席の証拠にしない**（v0.21・2026-09-17）。v0.19 は人の声（STT の書き起こし）から 60 秒を証拠にしたが、カメラが誰も
見ていないのに「こんにちは」の書き起こしへ返事した（17:03）。近くのテレビ・物音・聞き違いを声として拾い、声か音かの識別は
マイクでは成り立たない。代わりに**自分の発話**（`_last_said_at`・つなぎも含む・`DIF.speak` の直後に打つ）と **`/speaker`**
（`_speaker_set_at`）を 60 秒の窓で数える——話してよかった状態（相手が居た）はしばらく続く、打った人はそこに居る。
人の発話の印 `_last_human_at` は残るが、使い道は「ひとりの回数」のリセット（情-d）だけ。窓は全部 60 秒（`PRESENCE_SAID_SEC`・
`PRESENCE_EXPIRE_SEC`）、カメラの滞留窓だけ 180 秒（`CAMERA_PRESENCE_WINDOW`・カメラは 1 箇所しか見ないので長めに）。
誰も見えないときの会話入力は**入口で止め**（`_swallow_if_unheard`・情-h と同じ器）、人が映った最初の求めに列挙して主LLM が判断する。同時に SEEKING を押し上げて見に行く理由にする（`発火_mood` §2-c・案ア）。返事を作ってから溜める `pending_speech` は機器の知らせ専用。
**在席表の失効**：センサが「誰も居ない」を `PRESENCE_EXPIRE_SEC`（60 秒〔仮〕）見続けたら（`/speaker` を打ったらそこから数え直す・知-s・2026-09-18）、T の在席の刻み
（`Tonic._expire_presence_table`）が在席表の全員を `PersonMemoryManager.mark_absent` で消す。**話者の指定にも寿命がある**（知-t・v0.23・2026-09-18・09-17 の「残す」は撤回）：

| 「分かっている相手」（`agent.speaker_known()`・1 箇所） | 真になる条件 |
|---|---|
| `/speaker` から `PRESENCE_SAID_SEC`（60 秒）以内 | 打った人はそこに居る |
| その人に自分が返事してから 60 秒以内（`_speaker_confirmed_at`・起点が発話） | **会話中は切れない** |
| 在席表（顔照合）にその人が居る | 登録が済めばこれが主になる |
| **誰か分からないときの注記**（出-ae(1)・v0.28・2026-09-19）：`(present :speaker "unconfirmed" :note "誰か居るが、誰かは分からない。名前で呼ばない（直近のやりとりの名前も当てにしない）。知りたければ聞いてよい")`。事実だけの注記では、調停が W の直近に残る名前で「パパ、おかえりなさい」と呼んだ（13:14）。直近の窓に時間の上限を置く案（出-ae(2)）は別に設計 | 調停と主LLM に同じ文 |
| （GUI）**パネルはセンサの読みと話者の根拠を出す**（環-p-ろ・v0.27・2026-09-19）：「センサ：押入 に 2 人（12 秒前）」（`PresenceSensor.last_reading`）と「話者の根拠：/speaker から 45 秒（あと 15 秒）／返事から N 秒／在席表／切れている」（`agent.speaker_basis`・`speaker_known` と同じ順）。在席表（誰か）だけを映していて、センサが人を見ているのに「在席者なし」に見えた（実機 11:34） | 表示だけ・判断は変えない |
| **声の名乗り**（知-w・v0.26・2026-09-19）——**在席（カメラ）があるときだけ** | 「パパだよ」を調停が `speaker_claim` に読み、機械が在席と家族の名前を検めて `/speaker` と同じ効き（`set_active`・`_speaker_set_at`・PMM 同期）。在席が無ければ名乗りは入口で飲まれたまま（マイクは在席の証拠にしない・変えない）。家族に無い名前は付けない。道具の帰りの反復では読まない。**在席が無くて使えなかった名乗りは 30 秒預かる**（知-w-ろ・v0.30・`PENDING_CLAIM_SEC`）：見回りの帰りに名乗りを読んだ 6 秒後にセンサが人を見た（実機 13:27）。人を見た最初の求め（`_begin_request`）で生きていれば付け、過ぎていれば捨てる。テレビの名乗りの直後に人が入る誤りは短さで受ける |

どれも無ければ「誰か分からない」：T の tick が名前（`PersonRegistry`）と id（`PMM._speaker_id`）を戻し（`clear_speaker`・
ログ `話者の指定が切れた`）、想起は共通の面（`_active_memory` → 自分）、system 文に `(present :speaker …)` を出さない
（名前で呼ばない・「人は居るが誰かは不明」）、面の材料に立てない（`_conversation_perspective`）、話者ゲートとタイマーの
`speaker` も空。実機 2026-09-18 12:34：`/speaker` から 2 分 22 秒・在席表は空・センサは「誰か」——誰か分からない相手を
パパの面で想起し「パパ」と呼びかけ、版に「パパに聞かれ」と書いた（出-v の根）。`person_left`（両方消す）とは別の口。センサが無い構成では失効しない（在席表が唯一の情報源）。
返事（起点が発話）は `push_utterance` が付ける声の印で通る。判定の全体は `イベント駆動ループ` の
「話す／話さない（To-Be）」の図。

### 3-3. 在席は pose 条件付き
- PTZ カメラは一度に1方向しか見ない。**単一フレームの不検出＝部屋が空、ではない**。
- **見回りの行き先**（知-c・2026-09-17）：`PresenceMap.stale_order()`／`stalest_pose()`（定点ごとの最後に見た時刻・センサの走査が更新）が、情動が起点の求めの `[いま]` に「見ていない順：窓 12 分・テレビ 未…」として載り、調停が `look` を選んで行き先を書かなければ機械で最も長く見ていない定点を入れる（`_fill_look_target`）。08-01 の「印が無い定点は W に浮かない」はこれで解けた。
...
- **新しさ（t 軸）の若返り＝recall_count・last_recalled_at（017）の役割再編**（A-3 の Phase 1 残務・決定のみ・コードは Phase 2）：この二列は現在、旧 recall の `_compute_final_score`（`final_score = cosine × time_score × importance`）と `_mark_recalled` の中だけで使われる（grep 確認・機械想起 conversation で `recall_count += 1` と `last_recalled_at = now()`、spontaneous で `last_recalled_at` のみ、system は無更新）。新設計での対応づけ：`recall_count`＝新しさ（t 軸）の若返り回数（`time_decay` の reinforce＝半減期倍化）、`last_recalled_at`＝若返りの起点リセット。**根づき の n（評価由来の正味デルタ）とは別系統で、想起回数からは引き継がない**。更新トリガは機械想起からフルLLM の参照申告へ移す（[D-想起合成]「機械想起では 根づき も freshness も触らない」）。二列は旧 recall スコアリングに閉じているため、Phase 1 では現状維持（旧 recall が読む・外部挙動不変）とし、再編のコードは 5軸スコアラを載せる Phase 2（t 軸の若返りと a 軸の (a0,n)）で行う。§1-3 の「減衰エンジン（流用可）＝time_decay.py（reinforce で半減期倍化）」がこの若返りの実体。
  ...
  **`last_recalled_at` そのものは 044 で `situated_memories` へ移る**（**関係の面ごと**の量に
  なる。「人ごと」ではない＝記憶はすべてパジュの体験で、`person_id` は所有者でなく誰と関係する
  面かを表す）。取込 `groundedness_g0`（**パジュにとっての驚き**）は出来事1件につき1つなので
  `observations` に残る。
...
`facebook/dinov2-small`（ViT-S/14・384次元）。推論は初回 4.9 秒（重み取得込み）、**以降 19 ミリ秒**。

**同じ画像で 0.0000、別の定点（pan 22°差）で 0.4167、同じ向きで高さ違い（tilt 7.5°差）で 0.4485。** tilt の 7.5° が pan の 22° より見えを大きく変える。定点を tilt 違いで分ける判断（しきい値 0.02）は、見えの上でも裏付けられた。

**隔たりの分布（138 件・実機）**：最小 0.0020、中央 0.0060、90% 0.1360、95% 0.2200、最大 0.4510。「いつもどおり」は 0.006 前後に集中し、変化があると 0.1〜0.45 へ跳ねる。桁で分かれており、min-max 係数を決めるのに足りる分布である。ただし **$\widehat{S}=\max(\widehat{S}_{在席},\widehat{S}_{景色})$ なので、在席側が揃うまで係数は決められない。**

「普通」は定点ごとに `pose_norms`（`vector(384)`・マイグレーション 034）へ置く。部屋の映像から作る値なので `agent_state` の雑多なキーバリューには混ぜない。**育つまで（5回）は距離を出さない**（最初の1枚をそのまま「普通」にすると、そのとき写っていたものが基準になる）。
...
> v0.28：§3-2b 誰か分からないときの注記に「名前で呼ばない」（出-ae(1)・2026-09-19）。
> v0.27：§3-2b パネルにセンサの読みと話者の根拠（環-p-ろ・2026-09-19）。
> v0.26：§3-2b 声の名乗りで話者を付ける（在席があるときだけ・知-w・2026-09-19）。
> v0.25：§3-3 静止物を人と数えない（知-v・2026-09-18）。
> v0.24：失効は `/speaker` から数え直す（知-s・2026-09-18）。
> v0.23：**話者の指定にも寿命**（知-t・`speaker_known` の 3 条件・切れたら共通の面・名前で呼ばない・2026-09-18）。§3-2b。
> v0.22：不在の会話入力は入口止め（情-h の器）（2026-09-17）。§3-2b。
> v0.21：**マイクは在席の証拠にしない**——自分の発話と `/speaker` を 60 秒・滞留窓 180 秒・見回りの行き先（`stale_order`）（2026-09-17）。§3-2b・§3-3。
> v0.19：**居るかの正本は在/不在の層・在席表は「誰か」だけ・失効 60 秒・声の窓 60 秒**（知-p・2026-09-17）。§3-2b に追記。
> v0.18：**§3-2b「誰かがいる」と「知っている人がいる」**（2026-09-13・知-h）。配信ゲートと主LLM への在席を、居るか → 誰か の順に。
> v0.17：**意味づけを二段にした**（2026-09-12）。即席＝YOLO のラベルで見た印を書き、VLM は背景で差し替える。主LLM は写真そのものを受け取る（`イベント駆動ループ` v0.43）。§3-2 の意味づけ行を更新。
