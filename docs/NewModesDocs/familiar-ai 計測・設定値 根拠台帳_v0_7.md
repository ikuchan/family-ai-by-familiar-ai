...
> v0.6：**中心化実装後の実分散を計測**（2026-07-18・本番 obs_embeddings 2,525件・読み取りのみ）し §3 を改訂。中心化で無作為ペア p50 が 0.4800→−0.0235、窓幅が 0.0918→0.2427（約2.6倍）に健全化し、2026-06-29 の計測を再現した。値域が [0,1] にほぼ収まるため **c_lo=0.0・c_hi=1.0**（伸長式は実装するが係数は効かせない）と決めた。式を残す理由は c_lo/c_hi が Config 調整可で今後変える可能性があるため。§1 に中心化の実装状況を追記。
