...
**1回のフル LLM 呼び出しで、複数の仕事の結果をまとめて JSON で受け取る。** 内省は蒸留だけでなく棚卸しや自己調整も担うので、仕事ごとに LLM を呼ぶのではなく、1度でまとめて片付ける。器は後から仕事を足せる形にする。調停（`arbiter.py`）が同じ方式で、JSON を返させて拾っている。
...
| LLM | **フル LLM を1回**。既存の日次要約は軽量LLM だが、内省は他の仕事も同時にやるので質を取る |
| 出力 | 自己エピソード（`kind='day_summary'`）と、**畳む対象の id 配列** |
...
いま登録したのは `MemoryConfig.distill_min_a0`（既定 0.47・範囲 0.20〜0.70）の1件だけである。**内省が値を提案する部分は記-a-に で足す。** 蒸留を回して分布が見えてから決めるほうが確実で、いま作ると根拠のない値を根拠なく動かすことになる。
