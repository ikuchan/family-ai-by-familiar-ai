...
- **1つの発言に2回答えた**（2026-07-30・GUI）。キューに2件入っており、ターンが2回走った。**原因は特定できていない。** GUI には入口が2つある（キーボードと音声）が、**音声側がログを何も出していなかった**ため区別できなかった。入口の名前を添えたログを足し、短い間の同じ入力を落とすようにした（既定 3 秒・`INPUT_DEDUPE_WINDOW_SEC`）。**言い直しは弾かない**（窓は受け入れた時刻から測る）。`REALTIME_STT=true` で音声入力が動いていたので、マイクが何かを拾った可能性が高い。次に起きたときはログで入口が分かる。
