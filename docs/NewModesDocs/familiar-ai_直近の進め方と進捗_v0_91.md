# familiar-ai 直近の進め方と進捗（v0.91）

> Phase 1（O/MI モデルの確立）の作業について、直近の進め方と現在地の記録。詳しい方法論は「出力に関する指示」に、段取りは課題8 に、値の根拠は課題5 と計測台帳にある。

**この文書の読み方**（2026-09-10 に明記）。中身は3つに分かれ、鮮度が違う。

| 節 | 何か | 鮮度 |
|---|---|---|
| 進捗（段階A〜C・Phase 2） | **Phase 1・2 の記録**。当時の到達点 | 古い。**いまの現在地ではない** |
| 次の一歩 | 2026-07〜08 の判断の並び | 古い。**いまの次の一歩ではない** |
| **保留・申し送り** | 実機や運用で見つけて未決のもの | **生きている**。ここが本文である |

**いまの現在地は `課題8_段取り設計` §7 の着手順にある**（2026-09-10 時点の「いまここ」は
知-c＝昼夜の見回りで、実装済みだが**首が実際に回ることを一度も確かめていない**）。
...
4. コードとテストを当て、`ruff`・`mypy`・全体テスト（`./scripts/run_tests.sh`）を回して緑にする。
   （2026-09-10 訂正：以前は「ユーザーが当てて回す」と書いていたが、いまはこちらが直接
   リポジトリを編集して回す。`.claude/rules/作業手順.md` の但し書きと揃えた。）
...
- **GUI に渡した発話が画面に出なかった（2026-09-12 実機・未解決）。** 「何が見えますか？」
  への返事「目の前を見ますね。」は `chat.log`（表示関数 `append_line` の入口）には
  14:56:40 に在るが、画面には出なかった。同じ経路を通った 14:56:20 のつなぎは出た。違いは
  前者が GUI のターン終了後に駆動体から来たこと。当時のログでは「渡した」までしか分からず、
  吹き出しを足したか・足したあと画面内に来たかが見えなかった。

  **観測点を足した**（同日）。`ChatLog._add_bubble` が足した直後に
  「GUI 吹き出し N件目を足した スクロール=値/最大」、Qt が1周まわったあとに
  「描いたあと 可視=はい/いいえ」を `app.log` に残す。駆動体→GUI の通知（`_emit`）で
  表示先が例外を出したら握りつぶさず `logger.exception` で残す。次に同じことが起きたら、
  この3行で (a) 足したが画面外、(b) 足す前に落ちた、が分かれる。

- **l10n 68本の再生成は、いまはやらない**（2026-09-12 に決めた）。英語 README が
  2026-09-10 に11行変わり（旧 ReAct 経路を「how it works」として案内していたのを直した）、
  `readme-l10n/` の74本はすべてその前の状態にある。

  **実害は小さい。** 68言語の読者が `docs/technical.md`（旧版）へ案内されるが、**その文書は
  消えておらず `docs/OldDocs/` に在る**のでリンクは切れていない。

  **やると全文が置き換わる。** `scripts/generate_l10n.py --only-readme` は README 全文
  （20,018 字・約 6,000 トークン）を1言語1回で訳し直す。68回・`claude-haiku-4-5` で
  **約 370〜450円**。変わったのは11行だけだが、差分だけ訳す口は無い。

  **訳す内容そのものが微妙である。** 直したのはリンクの案内文で、リンク先の
  `docs/NewModesDocs/` は**日本語のみ**である。68言語の読者へ「日本語の設計文書を見て
  ください」と案内する文を、68言語へ訳すことになる。

  **英語 README が落ち着いてからまとめて回す。**

- **本番 DB へ未適用のマイグレーションが3本ある**（2026-09-10 に書き留めた。それまで
  どの文書にも残っていなかった）。テスト DB では通っているが、本番（`familiar_ai`・5432）は
  当てていない。**適用は人の手で行う**（この作業は本番への書き込みを禁じられている）。

  | 順 | file | 中身 | 備考 |
  |---|---|---|---|
  | 1 | `2026-09-05-057_reset_pad_to_unmeasured.py` | 旧目盛りで書かれた PAD を未測定へ戻す | 当てたあと `scripts/remeasure_emotion_pad.py --apply` を回す（**約25円・11分**） |
  | 2 | `2026-09-06-059_move_supersede_into_relations.py` | `observations.superseded_by` を関係へ移し、列を落とす | 058（`relations` の器）が先に要る |
  | 3 | `2026-09-07-060_move_wr_into_relations.py` | 共起（WR）を関係の器へ移す | `wr_records` を落とす |

  **順序を守る。** 059 は 058 が作る器へ移す操作で、060 は 059 のあとに来る。
...
- **見回りが実機で成立しなかった**（2026-08-01）。Drive は発火し（`Drive fired: seeking`）、`see` は動いて意味づけも通ったが（`出入り口で見えたもの 2 件：boy、man`）、**`look` が一度も選ばれず、1時間半で 4 回とも同じ定点だった**。定点は 3 件登録され（窓側・出入り口・襖側）、PTZ も繋がっており、内声も `see・検索・look などの具体的な行動を1つ選んで実行` と `look` を名指ししている。それでも選ばれない。

  **選ばなかったのではなく、選ぶ理由が無かった。** 在席センサは終始「出入り口に 2 人」を報告し、カメラは最初からその向きを見ている。探索したいときに目の前に人が写っていれば `see` で足り、他の定点へ首を振る動機が無い。`ユースケース③` の定める「最も薄れた印の定点を選ぶ」は、O に「窓側を見た」「襖側を見た」の印が溜まって初めて働くが、その印が独立した記録として残っていなかった。

  **見た印を鎖の外へ出した**（`direction="観察"`・畳まない・W へも載せる）。旧 `run()` はこれを書いており、本番に 256 件あって 2026-07-24 で途絶えている。新しいループへ移るとき `camera_used=False` が渡るようになって到達しなくなっていた。同じ意味の記録なので `direction` は分けない。旧との違いは、定点名が頭に付くことである。実機で 8 件が書かれ、いずれも畳まれずに残ることを確かめた。

  **ただし `look` はまだ選ばれない。** 印はすべて「出入り口」なので、薄れの順序が付かない。見ないから印が無く、印が無いから見に行かない。「一度も見ていない定点」をどう扱うかは未決である（`presence_map.stalest_pose` は「見回りが使う」と docstring に書きながら呼び出し元が 0 件で、実際の定点選択は O→W 側にある）。

  **反復の使い切りも実在する。** 1 回目の自発ターンは `recall` を 3 回選び、うち 2 回が「すでに調べた語」で弾かれ、`see` に届いたのが 4 反復目で、そこで上限（5）に達した。**弾かれた反復も 1 手として数えられる。** 印の有無とは別の問題として残っている。

- **`see` が見えたものを言葉にできていなかった**（2026-08-01）。知覚スライス S1 は完了と記録していたが、VLM 返答のコードフェンスでパースが落ち、`see` の完了に何が写っていたかが載らなかった。テストは偽のバックエンドを使うので通り、実機ログの「意味づけは何も返さなかった」は `info` 一行で埋もれていた。**偽の返答が本物と同じ形をしていないと、テストは通っても実物は動かない。** 実機の返答をそのまま見る道具として `scripts/probe_see.py` を置いた（カメラで1枚撮り、VLM の生の返答と判定を出す。DB には触らない）。詳細は `課題8 §7` の S1 にある。

- **全体テストの並列度を4へ下げた**（2026-08-01）。埋め込みモデル（bge-m3）はワーカーごとに読み込まれ、GPU で 1.7〜2.3GiB、CPU で 1140MiB を占める。この機体は VRAM 11.63GiB・RAM 15GiB（swap 4GiB は常時ほぼ満杯）なので、論理CPU数の12ワーカーではどちらの資源でも足りず、`CUDA out of memory` かプロセスごとの即死で `worker 'gwN' crashed` になっていた。載せる先を CPU へ移しても解決せず（所要が 62 秒から 182 秒へ伸びたうえワーカーが落ちた）、同時に載る数を抑えるほうが本筋だった。4 は VRAM から決めた（2.3GiB × 4 = 9.2GiB）。載せる先を選ぶ `EMBEDDING_DEVICE` は、資源の違う機体のために残してある（既定は自動）。
...
> v0.91：**知-n（タイマー）完了**（2026-09-15）——道具 3 本・表 `timers`・T が鳴らす・確かめたものだけ通り抜け・`/timer stop`。
> v0.90：**知-g-い・知-g-ろ 完了**（2026-09-15）——Vault に聞く動作 `vault`（本人のターンだけ）と、パジュ宛てのメモを 1 時間に 1 回読んで変化で動く。ObsidianMemo 側は `get_notes_for_paju`（2 コミット・未 push）。
> v0.89：記-b を消し、`relationship_state` を撤去（064）。束 A・B は机上で閉じられるものが無くなった（2026-09-15）。
> v0.88：**記-k・記-l 完了**（2026-09-15）——層 3 の最小標本数、層 1 ①の材料を「まだ畳まれていない」に改め 1 晩 10 回の上限。次は実機で REST を 1 回（記-j）。
> v0.87：**情-g・環-l 完了**（2026-09-15）——午前の実機ログで見つかった 2 つ（自発ターンがひとりの回数を戻す・`complete()` が思考ブロックで空）。記-k・記-l は仮値待ち。
> v0.86：**環-d の旧 15 欲求を撤去**（2026-09-15・063）。欲求は 5 軸だけ。
> v0.85：**記-a-ろ-に 完了**（2026-09-15）——核の同一を 1 件に、超過ぶんを意味の近い束（τ の覆い＋容量）ごとに `まとめ` へ固める。REST の層 1 は 4 段が揃い、残りは実機確認（記-j）。
> v0.84：**記-a-ろ-ろ 完了**（2026-09-15）——REST 内省の層 1 が①の前に使われる情報量 $I$ を測り、$I>I^\*$ なら参照されなかった核の根づきを $\Delta$ 減らす。次は ろ-に（核の固め）。
> v0.83：**出-p・知-f・環-d（今回分）完了**（2026-09-14）——家族の予定は期間つきの求め、話者ゲート、読み手の無い 5 表の DROP（061・次回起動で適用）。実機の表は `課題8` 束 D。
> v0.82：**記-a-へ・記-a-に・記-a-と 完了**（2026-09-14）——REST 内省の 1 パスが 4 層（出来事①→自己像→設定値→能力）を通して回るようになった。実機では記-j（1 パスを 1 回確かめる）が先。次は ろ-ろ（$\Delta$・$I$ の計測）。
> v0.81：**記-a-ろ-は 完了**（2026-09-14）——REST 内省の 1 パスが層 1 の①（日次の畳み込み）を回すようになった。実機で誰も居ない晩に初めて動く。次は ろ-ろ（$\Delta$・$I$ の計測）。
> v0.80：**現在地＝記-a-ろ-は（①日次の畳み込み・専用ループの最小形）**（2026-09-14）。同日までに 4 層の定義（`用語一覧` v0.70）、層 1〜3 の設計方針（3 本）、記-a-ろ-い（$HL$ 10 日・強化B 廃止・関連想起の 2 並び）、情-f（内部状態の言葉）、出-o（道具の失敗）、記-i（計測ログ）、記-h（直近の枠）、出-n 1〜4 を終えた。
> v0.79：**REST 内省を 4 層で定義し直した**（2026-09-14）——出来事（畳む）→自己像（抽象化する）→設定値（調整する）→能力（再定義する）。目的は自己認識の変革。設計方針は層ごとに新規文書で書く（出来事・自己像・設定値の 3 本）。`self_narrative` は層 1 で代替、棚卸しは消す方向。
> v0.78：**l10n 68本の再生成を「いまはやらない」と決めた**（2026-09-12）。英語 README の
> 変更は11行で、`readme-l10n/` の74本は古いが**リンクは切れていない**（旧版は `OldDocs/` に
> 在る）。やると全文が置き換わり約 370〜450円で、訳す内容もリンクの案内文である。
> 英語 README が落ち着いてからまとめて回す。

> v0.77：**本番 DB へ未適用のマイグレーション3本を申し送りへ書いた**（2026-09-10・群H の
> 点検）。057・059・060 はテスト DB では通っているが本番には当てていない。**どの文書にも
> 残っていなかった**ので、会話の中だけで受け渡されていた。

> v0.76：**節ごとに鮮度を明記した**（2026-09-10・群B の点検）。進捗（Phase 1・2）と
> 次の一歩は当時の記録で、**いまの現在地ではない**。生きているのは 保留・申し送り である。
> 進め方の 4 番が「ユーザーがコードとテストを当て、全体テストを回す」のままだったので、
> `.claude/rules/作業手順.md` の但し書きと揃えた。差し替え待ちの2件も解消済みと書いた。
...
> v0.72：**I のコンポーネントを再整理した**（2026-08-01）。出入り口を IIF・DIF・AIF・OIF の4つに限り、外部を身体・MCP-音楽・MCP-検索・主LLM の4つへ束ねる形を決めた（`設計図` ③-2）。3つのキューは IIF の1本へ統合し、同期にするのは記憶と軽量LLM だけとする。段取りは環-e（挙動不変・OIF → DIF → 実体の切り出し）と環-f（挙動が変わる・IIF と主LLM の非同期化）に分けた。用語一覧へ IIF・OIF・ULM を追加した。

> v0.71：**知-c の実機確認の結果と、環-e の新設**（2026-08-01）。見回りは成立しなかった。Drive は発火し `see` も意味づけも通ったが、`look` が一度も選ばれず 4 回とも同じ定点だった。原因は見た印が独立した記録として残らず、薄れの順序が作れないことで、印を鎖の外（`direction="観察"`）へ出した。実機で 8 件が畳まれずに残ることを確かめたが、印がすべて同じ定点なので `look` はまだ選ばれない。あわせて、**I の切り出しを 環-e として立てた**。設計は資源ハンドラと動作器を別の実体と定めるが、実装では `loop/event_loop.py` が身体と外部サービスを直接叩いている。挙動を変えず移動だけを行う課題で、着手順の位置は未定である（`課題8 §7` v0.49・`モジュール分割設計` v0.7）。

> v0.70：**着手順を組み替えた**（2026-08-01）。知-c（見回りの実機確認）を先頭へ、知-d（顔と声の登録）をその次へ、記-a（REST 内省）をそのあとへ動かした。REST は誰も居ないときにだけ回る機構で、人が居るあいだは実機で確かめられない。実機が使える時間のほうが希少な資源なので、それを要する課題を前に置く。「次の一歩」を差し替え、蒸留の未決4点を明記した。順序の正本は `課題8 §7` v0.48。

> v0.69：**`see` の意味づけを実際に通した**（知-a S1 の積み残し）。`extract_entities` が VLM 返答のコードフェンスでパースに失敗し、`see` の完了に何が写っていたかが載っていなかった。実機で確かめると、llava:7b は同じ画像・同じプロンプトでも回ごとに包む・包まないが変わる（素の JSON で `chair`・`person`、フェンス付きで `child`・`adult`・`chair`）。剥がす処理を `core/helpers.strip_code_fence` に一本化した。あわせて、ターン後の `SceneTracker.update` 呼び出しと `_react_to_scene_events` を撤去した（渡っていたのはカメラ画像ではなく自分の発話テキストで、そこから拾ったラベルで `greet_companion` が跳ねる経路だった）。全体テストの並列度も4へ下げた（ワーカーの資源枯渇による即死を止めるため）。申し送りへ2件を追加。着手順は変えていない（記-a が現在地）。

> v0.68：**求めの版チェーンを実装**した（V1〜V4・`設計方針_求めの版チェーン` v0.3）。`superseded_by` を版履歴だけの意味へ戻し、`close_with_children` を撤去した。求めは `direction="求め"` の1本の鎖として進み、並行する調査は通し番号で content に列挙する。同じ語はこの求めのあいだ二度と調べない（実機で同じ `recall` が4反復続いた）。発話の記録には「検索を始めた」を一度だけ追記し、埋め込みを作り直す。W 構築の統一の S5 は、この設計へ差し替えて完了とする。

> v0.67：**W 構築の統一の S4 を実装**した。手組みの連結を撤去し、取込 O を含めて候補集合の1本の経路で W を組む。量は $K$（7件）と字数枠（`workspace_max_chars`・既定 40000）で抑え、1件の途中では切らず適合度の低い件から丸ごと落とす。S5（畳む処理）は、`close_with_children` が`superseded_by` に2つの意味を載せていることが分かったため、**求めを1本の版チェーンにする設計へ差し替える**（未着手）。
