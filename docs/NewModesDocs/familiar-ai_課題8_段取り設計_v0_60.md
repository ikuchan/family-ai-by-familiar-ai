# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.60）
...
| 記-d | 読み手の居ない列と表の撤去（`scope`・`person_id`・`importance`・`unfinished_business`） | **実行済み（2026-08-03）**。着手順の最下段だったが、この表を書いた翌々日に手が付いた。**復元も完了**（039・040・041・042／2026-08-31〜09-01） |
...
**`person_id` は落とした**（2026-09-01・042 として復元）。復元にあたって実測したところ、上の
「行き先を決める」という条件そのものが成り立たなくなっていた。

**所有者絞りは、そもそも人を分けていなかった。** 2026-08-03 のダンプで `observations` 5080 行の
うち 4904 行（96.5%）が既定値 `default` のままで、家族4人のうち2人（いくながこうき・いくなが
たえこ）は所有行を1件も持たない。010 が `NOT NULL DEFAULT 'default'` で入れた列を、書き込み側が
文脈の person のまま埋め続けた結果である。

**人ごとの区別は situated 側が担っている。** `situated_memories`（当時は `situated_embeddings`）
には `default` の行が1件も無く、`__self__` 6805・いくながゆうすけ 1397・いくながたいき 368・
いくながこうき 69・いくながたえこ 43 と、実在の人物だけが並ぶ。

**C-1 が残した役目は、母集合が 0 行だった。** 「situated 行を持たない観測」は、8月21日時点の
生存する観測 2672 件のうち **0 件**である。全件が situated 行を持つ。フォールバックに所有者絞りを
残すと、最後の網が `default` 以外の person で恒常的に空になるだけで、残した役目は果たせない。
よって **フォールバック二関数からも所有者絞りを外した**。

**重複判定の30秒窓だけは絞りを保ち、`writer_id` へ移した。** 重複とは「同じ書き手が同じ内容を
...
**撤去の途中で、本番の不具合が1件見つかった。** `recall_self_model` と `recall_curiosities` は
`person_id=AGENT_SELF_ID`（`...0000`）で絞っていたが、書き込みは文脈の person で入るため、
8月3日時点の `self_model` 958 行・`curiosity` 238 行の**すべて**が `default`（`...0001`）だった。
一致する行が無く、**両関数は本番で常に空を返していた**。テストは fixture が `AGENT_SELF_ID` を
明示して行を植えていたので通っていた。**書き込み経路と食い違う fixture は、絞りの誤りを隠す。**
042 は所有者絞りごと外すので、この食い違いも同時に消える。
...
> v0.60：**記-d の `person_id` を撤去し（042 の復元）、C-1 の申し送りを実測で取り消した**
> （2026-09-01）。C-1 は「フォールバックは所有者絞りのまま『situated 行を持たない観測』を拾う
> 役目を残す」と決めていたが、その母集合は 0 行だった（生存する観測 2672 件すべてが situated
> 行を持つ）。所有者絞り自体も 96.5% が `default` に潰れており人を分けていない。よって
> フォールバック二関数からも外し、重複判定の30秒窓だけ `writer_id` へ移した。撤去の途中で、
> `recall_self_model` と `recall_curiosities` が本番で常に空を返していたことが分かった
> （絞りが `__self__`、書き込みが `default`）。**記-d の復元はこれで完了**（039・040・041・042）。
