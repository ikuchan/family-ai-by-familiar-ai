...
> v0.93：**2026-09-16 の完了分をまとめて記す**（09-17・進捗ログの抜けを埋めた）——**環-m 完了**（列から取り出す口は `_take_trigger` の 1 つ・会話入力は保留箱より先・12:40 の 88 秒止まりの直し）、**情-h 完了**（沈黙は入口で止め、明けたら 1 つの求めにまとめる・`core/silence_hold`・案イ撤回）、沈黙の解き方と守り（本人の「話していいよ」で解く・名前が無ければ掛けない・`core/silence_rules`）、**知-m ③ 実装**（`look` に相対の向き・右＝pan 負・向き終わるまで待ってから撮る・look の帰りを see と同じ扱い）、機器の知らせは返事ではない（`[届いた知らせ]`・`_LEAD_DEVICE`）、タイマーの追加（`TIMER_VOICE_GAIN` 鳴った知らせだけ声を大きく・`TIMER_SILENCE` 掛けているあいだは黙る・起点は人が言った瞬間・実機 4 件）、声と耳（`ELEVENLABS_MODEL`・既定 flash／`AUDIO_INPUT_GAIN`／GUI の `/reload`／声が出なかったら warning／`REALTIME_STT=on` も真）、GUI・TUI の沈黙モード撤去（読まれていなかった）、記-a-ろ ①に $a_0$ を効かせる（`distill_min_a0`）、環-l 追記（思考を明示的に切る）。**知-o**（マイクを閉じる）を C-8 に登録（未着手）。設計は `イベント駆動ループ` v0.66〜v0.73・`設計方針_タイマー` v0.1 追記・`ユースケース③`・`音声入力からGUIへの経路` v0.4〜v0.5。
> v0.92：**骨格の整理**（2026-09-17）——Phase 1・2 の記録と 2026-07〜08 の「次の一歩」を `課題8_記録_完了した課題` v0.1 へ移し、この資料は「進め方」「保留・申し送り」「進捗ログ」の 3 節にした。更新履歴は進捗ログを兼ねる。
