# familiar-ai 設計詳細：活性・O書込・知覚在席（定数台帳・現状コード所在・移行）（v0.28）
...
|---|---|
| センサあり | `room_occupied()`（滞留窓 **180 秒**内に人）**or** 自分が話してから `PRESENCE_SAID_SEC`（60 秒〔仮〕）以内 **or** `/speaker` を打ってから 60 秒以内。**マイクで拾った声と在席表は見ない**（v0.21） |
