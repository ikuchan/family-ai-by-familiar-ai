# familiar-ai 課題8 段取り設計（段階的 TDD 改造の順序と依存）（v0.49）
...
| 環-d | #12b | — | 廃止ストアの移管と撤去 |
| 環-e | — | — | I の切り出し（2026-08-01 新設・旧番号なし） |
| 出-a | #9 | 順10 | 音声出力 |
| 出-b | #10 | — | 致命的エラー方針 |
...
| 知-e | D-rel（関係の加工） | 知-d |
| 環-e | I の切り出し（動作器・資源ハンドラ・生成器ほか） | 依存なし。**着手順は未定**（下記） |
...
**環-e を立てた**（2026-08-01）。設計は I を5つの実体（ループ管理・生成器・評価器・動作器・統合保守器）に分け、資源ハンドラ（LLM 担当・実行担当）と機器 IF を外に置く。実装では `loop/event_loop.py`（1,444 行）がそのうち4つを同居させ、カメラ・VLM・検索・記憶・軽量LLM・TTS を直接呼んでいる。**この課題は挙動を変えず、移動だけを行う。** 中身は `モジュール分割設計` v0.7 にある。

**環-e の位置は未定である。** 依存は無いのでいつでも着手できるが、切り出しの単位が決まってから順序を確定する。`see`・`look` からは始めない（知-c で挙動を追っている最中に同じ箇所を触ると、挙動の変化がどちらに由来するのか切り分けられなくなる）。
