# familiar-ai イベント駆動ループ（#11・段階5）（v0.55）
...
> v0.55：**沈黙依頼のあいだも、頼んだ本人以外への返事は通す**（2026-09-13・案イ）。同日、調停が選んだ MCP の道具にその道具が受け取る入力だけを渡す（`_tool_input_for`）・器の無い完了は打ち切られたものとして捨てる（停止ボタン・M）の 2 つの実機の穴も塞いだ。
