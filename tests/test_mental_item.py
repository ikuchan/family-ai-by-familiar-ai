...
def _insert_obs(
    ...
    cur.execute(
        ...
        "(id, content, timestamp, direction, kind, emotion, groundedness_g0) "
        "VALUES (%s, %s, %s, %s, %s, %s, %s)",
        (obs_id, content, ts, "unknown", kind, "neutral", groundedness_g0),
    )
    # 畳む印は関係にある（段 2）。
    if superseded_by:
...
def test_row_to_mental_item_builds_from_row() -> None:
    ...
    rows = mem._observations._read_observations_by_kind(
        kind="self_model",
        n=1,
        columns=("id", "content", "timestamp", "emotion", "groundedness_g0"),
    )
    row = rows[0]
    ...
    # 版の相手は列から来なくなった。読み手が渡したときだけ載る（段 2）。
...
def test_recall_self_model_returns_same_shape_newest_first() -> None:
    ...
    with conn.cursor() as cur:
        _insert_obs(
            cur, "sm-old", "old self model", "self_model", AGENT_SELF_ID, _NOW - timedelta(hours=1)
        )
