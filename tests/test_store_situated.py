...
（045 で視点ベクトル `perspective_vec` を落とした）。
...
def test_situated_vector_is_normalised() -> None:
    """ベクトルは長さ1（コサインを取る前提）。"""
    mem = np.array([1.0, 2.0, 0.0], dtype=np.float32)
    got = _situated_vector(mem, None)
    assert abs(float(np.linalg.norm(got)) - 1.0) < 1e-5


def test_situated_vector_subtracts_the_mean_when_given() -> None:
    """mu を渡すと中心化される（渡さなければ素のベクトルを正規化するだけ）。"""
    mem = np.array([1.0, 1.0, 0.0], dtype=np.float32)
    mu = np.array([1.0, 0.0, 0.0], dtype=np.float32)
    assert not np.allclose(_situated_vector(mem, mu), _situated_vector(mem, None))
...
def test_memory_module_no_longer_writes_situated_rows() -> None:
    ...
    # `perspective_vec` は 045 で層からも消えた（差は人でなく関係が作る）。
...
def test_situated_module_owns_the_vectors() -> None:
    ...
    for name in ("situated_memories", "embedding_means"):
