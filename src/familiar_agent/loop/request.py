...
class Request:
    ...
    just_returned: list[int] = field(default_factory=list)
    # 確認待ちの枠（出-y・`agent.confirm_frame`）。求めを始めるときに写し、W の最上部に載せる。
    confirm_frame: str = ""
    ...
    # 誰が `see` を出したか（`主LLM`／`調停`）。帰りの判断は出した側に返す（`_decide`）。
    see_by: str = ""
    ...
    # この求めを起こした欲求の軸（`seeking` など・情動が起点のときだけ・情-f）。内部状態の
    # 言葉で「発火：SEEKING（探索したい）」と明示するために持つ。会話・機器が起点なら空。
    fired_axis: str = ""
